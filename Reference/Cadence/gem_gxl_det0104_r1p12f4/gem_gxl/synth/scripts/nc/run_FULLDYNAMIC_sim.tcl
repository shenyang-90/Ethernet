...
#------------------------------------------------------------------------------
#
#    Primary Unit Name :      run_FULLDYNAMIC_sim.tcl
#
...
# purge irun log
set irun_logfile "${_SIMWORK_PATH}/irun.log"
...
foreach datafile $datafiles {
   ...
   if [string match *serial* $datafile] {
      ...
      exec irun -64bit ${tb_dir}/serial/VER.${testmode}.mainsim.v -update -linedebug -delay_trigger \
                ...
                -timescale 1ns/1ps \
                -log_ncvlog ${_SIMWORK_PATH}/ncvlog_${testname}_serial.log \
                -log_ncelab ${_SIMWORK_PATH}/ncelab_${testname}_serial.log \
                -log_ncsim ${_SIMWORK_PATH}/ncsim_${testname}_serial.log \
                -ncsim_args \
                +HEARTBEAT \
                +TESTFILE1=$datafile \
                $end_range \
      ...
      exec irun -64bit ${tb_dir}/parallel/VER.${testmode}.mainsim.v -update -linedebug -delay_trigger \
                ...
                -timescale 1ns/1ps \
                -log_ncvlog ${_SIMWORK_PATH}/ncvlog_${testname}_parallel.log \
                -log_ncelab ${_SIMWORK_PATH}/ncelab_${testname}_parallel.log \
                -log_ncsim ${_SIMWORK_PATH}/ncsim_${testname}_parallel.log \
                -ncsim_args \
                +HEARTBEAT \
                +TESTFILE1=$datafile \
                +FAILSET
