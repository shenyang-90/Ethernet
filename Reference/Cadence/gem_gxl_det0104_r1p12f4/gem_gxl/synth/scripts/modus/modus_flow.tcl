...
#    Primary Unit Name :      modus_flow.tcl
#
#          Description :      Template Script for Modus DFT Flow
#
...
if { [info exists TDRPATH] && ($TDRPATH != " ") } {
    set modus_tdrpath "$TDRPATH"
} else {
    set modus_tdrpath "$::env(Install_Dir)/defaults/rules/tdr"
}


if {$BLACK_BOX} {
    set black_box_args "yes"
} else {
    set black_box_args "no"
}


source $::env(Install_Dir)/bin/64bit/test_checks.tcl;
 set STOP_ON_MSG_SEV ERROR;   #includes ERROR|WARNING_SEVERE|WARNING
 set LOGDIR ${_ATPGWORK_PATH}/testresults/logs;

source $IPF_DESIGN_FLOW_SCRIPTS/modus/modus_clock_checks.tcl
...
set LINEHOLD_FILE $workdir/fullscan.test_cg_enable.linehold
...
foreach etlogfile [glob -nocomplain $workdir/testresults/logs/*] {
    file delete $etlogfile
}
...
build_model \
 -cell $DESIGN \
 -DESIGNSOURCE $netlist,$ATPG_OTHER_MODULES \
 -TECHLIB $ATPGLIB \
 -allowmissingmodules $black_box_args \
...
#CheckTSVlog $workdir/testresults/logs/log_verify_test_structures_${testmode}
  ...
  puts "Running verify_clock_constraints $testmode "
  verify_clock_constraints \
   -TESTMODE $testmode \
   -clockconstraints ${ATPG_CLOCK_CONSTRAINTS_FILE}

CheckTTUlog $workdir/testresults/logs/log_verify_clock_constraints_${testmode}
unset testmode
...
#The scanchain test for delay test is not different from the static scanchain test.
