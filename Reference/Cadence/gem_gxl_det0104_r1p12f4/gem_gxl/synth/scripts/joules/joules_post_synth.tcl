...
if {![file exists ${_JOULES_DATA_PATH}]} {
   file mkdir ${_JOULES_DATA_PATH}/postsynth
   puts "Creating directory ${_JOULES_DATA_PATH}/postsynth"
}

if {![file exists ${_JOULES_RPT_PATH}]} {
   file mkdir ${_JOULES_RPT_PATH}/postsynth
   puts "Creating directory ${_JOULES_RPT_PATH}/postsynth"
}
...
## Load Design
####################################################################
read_hdl  ${_OUTPUTS_PATH}/${DESIGN}.v.gz
...
report timing -lint > $_JOULES_RPT_PATH/postsynth/${DESIGN}_check_timing.rpt
...
report_clock_tree -name CT1 -out $_JOULES_RPT_PATH/postsynth/clk.rpt
}
...
write_sdb -out  $_JOULES_DATA_PATH/postsynth/${DESIGN}_vcd.sdb
    	...
    	foreach corner [get_lib_domains] { 
    		...
    		rtls_init
		plot_power_profile -unit $POWER_UNIT -png $_JOULES_RPT_PATH/postsynth/vcd_power_plot_$corner.png
    		compute_power -out  $_JOULES_RPT_PATH/postsynth/vcd_compute_power_$corner.rpt
    		report_power  -by_hierarchy -levels 1 -sort_by total -out $_JOULES_RPT_PATH/postsynth/vcd_report_power_$corner.rpt -unit $POWER_UNIT
		foreach frame [get_sdb_frames] {
		report_power -frames $frame -by_hierarchy -levels 1 -sort_by total -append -out $_JOULES_RPT_PATH/postsynth/vcd_report_power_$corner.rpt -unit $POWER_UNIT
		}
	} 
}
...
if { [ file exists ${RTL_TOGGLE_FILE_DIR} ] } {
  ...
  foreach power_state $POWER_STATES {
    ...
    #######################################################   
    write_sdb -out  $_JOULES_DATA_PATH/postsynth/${DESIGN}_${power_state_clean}.sdb
      
	foreach corner [get_lib_domains] { 
    		set_attr power_library [find /libraries -library_domain $corner] [find /libraries -library_domain WC]
    		set_attr default true [find /libraries -library_domain WC]
    		rtls_init
    		compute_power -out  $_JOULES_RPT_PATH/postsynth/compute_power_${power_state_clean}_$corner.rpt
    		report_power  -by_hierarchy -levels 1 -sort_by total -out $_JOULES_RPT_PATH/postsynth/power_${power_state_clean}_$corner.rpt -unit $POWER_UNIT
		foreach stim [get_sdb_stims] {
		report_power -stim $stim -by_hierarchy -levels 1 -sort_by total -append -out $_JOULES_RPT_PATH/postsynth/power_${power_state_clean}_$corner.rpt -unit $POWER_UNIT
		}
	} 
    set first_file 1
  }
...
report_activity  -out $_JOULES_RPT_PATH/postsynth/activity.rpt
report_ppa	 -out $_JOULES_RPT_PATH/postsynth/ppa.rpt 
#report_power	 -out $_JOULES_RPT_PATH/postsynth/power.rpt -append
...
report_icgc_efficiency -out $_JOULES_RPT_PATH/postsynth/icgc_efficiency.rpt 
