...
//----------------------------------------------------------------------------
// Description :
// This file contains the C test library for the cdn_gem_demo UVC.
//----------------------------------------------------------------------------

/*
 * File: cdn_gem_demo_c_test_lib.sv
 *
 * This file contains the C tests library for the cdn_gem_demo UVC.
 * Only the base test (defined here) is needed to run C tests.
 */

`ifndef CDN_GEM_DEMO_C_TEST_LIB_SV
  `define CDN_GEM_DEMO_C_TEST_LIB_SV

`ifdef CDN_DEMO_C

/*
 * test_description: cdn_gem_demo_c_base_test
 *
 * This is the cdn_gem_demo UVC base test (C wrapper).
 *
 * Class: cdn_gem_demo_c_base_test
 *
 * This is the cdn_gem_demo UVC base test (C wrapper).
 */
class cdn_gem_demo_c_base_test extends cdn_demo_base_test;
  ...
  `uvm_component_utils(cdn_gem_demo_c_base_test)
   ...
   * It is used to construct cdn_gem_demo_c_base_test objects.
   *
   * Parameters:
   *
   ...
   */
  function new (string name = "cdn_gem_demo_c_base_test", uvm_component parent=null);
  ...
  virtual function void build_phase(uvm_phase phase);
    ...
    for (int i=0; i<`CDN_DEMO_APB_NUM_OF_SLAVES; i++) begin
      base_addr[i] = 64'h0000_0000_fff3_0000 + i*64'h0000_0000_0000_2000;
      end_addr[i]  = 64'h0000_0000_fff3_0000 + (i+1)*64'h0000_0000_0000_2000 - 1;
   ...
   * hierarchy.
   * For the cdn_gem_demo_c_base_test, it sets the base address to the UVM_REG
   * default register map to match what is targeted by the C env.
   *
   * Parameters:
   *
   *    phase - The UVM phase object.
   ...
   * For the cdn_gem_demo_c_base_test, it overrides the super method to print
   * out a reduced the VE topology based on the UVM verbosity.
   *
   * Parameters:
   *
   *    phase - The UVM phase object.
...
endclass : cdn_gem_demo_c_base_test

`endif // CDN_DEMO_C

`endif // CDN_GEM_DEMO_C_TEST_LIB_SV
