...
uint8_t IsGem1p09(void *pD)
{
    switch(CEDI_PdVar(hwCfg).moduleId){
...
uint8_t IsEnstSupported(void *pD)
{
    switch(CEDI_PdVar(hwCfg).moduleId){
    case GEM_GXL_MODULE_ID_V1:
    case GEM_GXL_MODULE_ID_V2:
    case GEM_AUTO_MODULE_ID_V0:
    case GEM_AUTO_MODULE_ID_V1:
        return !CEDI_PdVar(hwCfg).exclude_qbv;
    default:
        return 0;
    }
}

uint8_t is2p5GSupported(void* pD)
{
    if (CEDI_PdVar(hwCfg).moduleId == GEM_GXL_MODULE_ID_V2)
        return 1;
    if (CEDI_PdVar(hwCfg).moduleId == GEM_AUTO_MODULE_ID_V1)
        return 1;
    return 0;
}

/* Also responsible for ability to set interrupt as fatal/non-fatal. */
uint8_t IsLockupSupported(void *pD)
{
    switch(CEDI_PdVar(hwCfg).moduleId){
    case GEM_GXL_MODULE_ID_V1:
    case GEM_GXL_MODULE_ID_V2:
        if (CEDI_PdVar(hwCfg).moduleRev >= 0x010a)
            return 1;
        else
            return 0;
    case GEM_AUTO_MODULE_ID_V0:
...
static uint32_t alignedToPtr(uint32_t size)
{
    ...
    uint8_t misalignment = (alignment - 1) & size;
    if (misalignment)
...
/* Initial declaration */
uint32_t emacGetMacType(void* pD, CEDI_MacType* macType);

static struct emac_regs *getMmslRegs(void *pD)
{
    void *ppD;
    if (((CEDI_PrivateData *)pD)->macType == CEDI_MAC_TYPE_EMAC)
        ppD = ((CEDI_PrivateData *)pD)->otherMac;
    else
        ppD = pD;
    return (struct emac_regs *)(((CEDI_PrivateData *)ppD)->cfg.regBase);
...
static uint32_t initTxDescLists(void *pD)
{
    ...
    CEDI_PdVar(txQueue[0]).vAddrList =
                (uintptr_t *)(pD + alignedToPtr(sizeof(CEDI_PrivateData)));
    for (q=0; q<CEDI_PdVar(txQs); q++)
...
static void initNetControlReg(void *pD)
{
    ...
    if ((config->ifTypeSel==CEDI_IFSP_2500M_SGMII)
        ...
        EMAC_REGS__NETWORK_CONTROL__TWO_PT_FIVE_GIG__MODIFY(regTmp, 1);
    else
        EMAC_REGS__NETWORK_CONTROL__TWO_PT_FIVE_GIG__MODIFY(regTmp, 0);
#endif
...
static void fillSupportedRasInterrupts(void *pD)
{
    CEDI_PdVar(hwCfg).supported_ras_int = 0;
    if (CEDI_PdVar(hwCfg).add_ecc_dpram)
        CEDI_PdVar(hwCfg).supported_ras_int |= CEDI_RAS_EV_ECC_RX_NCRR_ERR | CEDI_RAS_EV_ECC_TX_NCRR_ERR;
    if (CEDI_PdVar(hwCfg).add_dp_parity)
    {
        CEDI_PdVar(hwCfg).supported_ras_int |= CEDI_RAS_EV_DP_PARITY_ERR;
        if (CEDI_PdVar(hwCfg).tsu)
            CEDI_PdVar(hwCfg).supported_ras_int |= CEDI_RAS_EV_TS_PARITY_ERR;
    }
    if (CEDI_PdVar(hwCfg).add_csr_parity)
        CEDI_PdVar(hwCfg).supported_ras_int |= CEDI_RAS_EV_CSR_PARITY_ERR;
    if ((CEDI_PdVar(hwCfg).protect_tsu) && (CEDI_PdVar(hwCfg).tsu))
        CEDI_PdVar(hwCfg).supported_ras_int |= CEDI_RAS_EV_TSU_ERR;
    if (CEDI_PdVar(hwCfg).protect_descr_addr)
        CEDI_PdVar(hwCfg).supported_ras_int |= CEDI_RAS_EV_DESCR_ADDR_ERR;
}
...
uint32_t offloadsSupport(void *pD)
{
    if (pD==NULL)
		...
		|| (CEDI_PdVar(hwCfg).moduleId==GEM_GXL_MODULE_ID_V2)
        || (CEDI_PdVar(hwCfg).moduleId==GEM_AUTO_MODULE_ID_V0)
        || (CEDI_PdVar(hwCfg).moduleId==GEM_AUTO_MODULE_ID_V1));
}
...
uint32_t readDesignConfig(void *pD)
{
    ...
    reg = CPS_UncachedRead32(CEDI_RegAddr(designcfg_debug11));
    CEDI_PdVar(hwCfg).add_ecc_dpram =
            EMAC_REGS__DESIGNCFG_DEBUG11__ADD_ECC_DPRAM__READ(reg);
    CEDI_PdVar(hwCfg).add_dp_parity =
            EMAC_REGS__DESIGNCFG_DEBUG11__ADD_DP_PARITY__READ(reg);
    CEDI_PdVar(hwCfg).add_csr_parity =
            EMAC_REGS__DESIGNCFG_DEBUG11__ADD_CSR_PARITY__READ(reg);
    CEDI_PdVar(hwCfg).protect_tsu =
            EMAC_REGS__DESIGNCFG_DEBUG11__PROTECT_TSU__READ(reg);
    CEDI_PdVar(hwCfg).protect_descr_addr =
            EMAC_REGS__DESIGNCFG_DEBUG11__PROTECT_DESCR_ADDR__READ(reg);
    fillSupportedRasInterrupts(pD);
    CEDI_PdVar(hwCfg).lockup_supported = IsLockupSupported(pD);

    reg = CPS_UncachedRead32(CEDI_RegAddr(designcfg_debug12));
    cond = EMAC_REGS__DESIGNCFG_DEBUG12__GEM_HAS_CB__READ(reg);
    CEDI_PdVar(hwCfg).num_cb_streams = 0;
    if (cond)
        CEDI_PdVar(hwCfg).num_cb_streams =
...
uint32_t emacHandleGeneralInterupts(void *pD, uint8_t *handled)
{
#define CEDI_INT_STATUS_WR_CASE(Q) case Q: \
    ...
    uint32_t events;
    uint16_t dat16;
    uint8_t  qNum=0, cond1, cond2, cond3;
    CEDI_AnNextPage nullNp;
    uint8_t linkState;

    if (pD==NULL) return EINVAL;

    /* Test for RAS-related interrupts */
    isrReg = 0;
    if (CEDI_PdVar(hwCfg).lockup_supported)
        isrReg = CPS_UncachedRead32(CEDI_RegAddr(ras_int_status));
    if (isrReg)
    {
        *handled = 1;
        if (CEDI_PdVar(cb).rasEvent)
            (*(CEDI_PdVar(cb).rasEvent))(pD, isrReg);
        if (0==CEDI_PdVar(hwCfg).irq_read_clear)
            CPS_UncachedWrite32(CEDI_RegAddr(ras_int_status), isrReg);
    }
    ...
    if (isrReg) {
        ...
        *handled = 1;
        ...
        /************************ PHY MDIO Frame Tx'd **********************/
        if (EMAC_REGS__INT_STATUS__MANAGEMENT_FRAME_SENT__READ(isrReg))
        {
        ...
        cond1 = EMAC_REGS__INT_STATUS__TRANSMIT_UNDER_RUN__READ(isrReg);
            cond2 = EMAC_REGS__INT_STATUS__RETRY_LIMIT_EXCEEDED_OR_LATE_COLLISION__READ(isrReg);
        ...
        /*************************** RxFrame *******************************/
        if (EMAC_REGS__INT_STATUS__RECEIVE_COMPLETE__READ(isrReg))
        {
            /*vDbgMsg(DBG_GEN_MSG, 10,
        ...
        /************************ HResp not OK Event ***********************/
        if (EMAC_REGS__INT_STATUS__RESP_NOT_OK__READ(isrReg))
        {
            ...
            /************************* AN LP Page Rx ***************************/
        if (EMAC_REGS__INT_STATUS__PCS_LINK_PARTNER_PAGE_RECEIVED__READ(isrReg))
        {
        ...
        /************************* AN Complete *****************************/
        if (EMAC_REGS__INT_STATUS__PCS_AUTO_NEGOTIATION_COMPLETE__READ(isrReg))
        {
        ...
        /*********************** Link State Change *************************/
        if (EMAC_REGS__INT_STATUS__LINK_CHANGE__READ(isrReg))
        {
        ...
        /************************ Pause Event ******************************/
        cond1 = EMAC_REGS__INT_STATUS__PAUSE_FRAME_TRANSMITTED__READ(isrReg);
        cond2 = EMAC_REGS__INT_STATUS__PAUSE_TIME_ELAPSED__READ(isrReg);
        cond3 = EMAC_REGS__INT_STATUS__PAUSE_FRAME_WITH_NON_ZERO_PAUSE_QUANTUM_RECEIVED__READ(isrReg);
        ...
        cond1 = EMAC_REGS__INT_STATUS__PTP_DELAY_REQ_FRAME_TRANSMITTED__READ(isrReg);
        cond2 = EMAC_REGS__INT_STATUS__PTP_SYNC_FRAME_TRANSMITTED__READ(isrReg);
        ...
        cond1 = EMAC_REGS__INT_STATUS__PTP_PDELAY_REQ_FRAME_TRANSMITTED__READ(isrReg);
        cond2 = EMAC_REGS__INT_STATUS__PTP_PDELAY_RESP_FRAME_TRANSMITTED__READ(isrReg);
        ...
        cond1 = EMAC_REGS__INT_STATUS__PTP_DELAY_REQ_FRAME_RECEIVED__READ(isrReg);
        cond2 = EMAC_REGS__INT_STATUS__PTP_SYNC_FRAME_RECEIVED__READ(isrReg);
        ...
        cond1 = EMAC_REGS__INT_STATUS__PTP_PDELAY_REQ_FRAME_RECEIVED__READ(isrReg);
        cond2 = EMAC_REGS__INT_STATUS__PTP_PDELAY_RESP_FRAME_RECEIVED__READ(isrReg);
        ...
        if (cond1 || cond2)
        {
            ...
            /************************* TSU Event *******************************/
        cond1 = EMAC_REGS__INT_STATUS__TSU_SECONDS_REGISTER_INCREMENT__READ(isrReg);
        cond2 = EMAC_REGS__INT_STATUS__TSU_TIMER_COMPARISON_INTERRUPT__READ(isrReg);
        ...
        /************************* LPI Status Change ***********************/
        if (EMAC_REGS__INT_STATUS__RECEIVE_LPI_INDICATION_STATUS_BIT_CHANGE__READ(isrReg))
        {
        ...
        /************************* Wake On LAN Event ***********************/
        if (EMAC_REGS__INT_STATUS__WOL_INTERRUPT__READ(isrReg))
        {
        ...
        if (EMAC_REGS__INT_STATUS__EXTERNAL_INTERRUPT__READ(isrReg))
        {
...
uint32_t emacHandleQnInterupts(void *pD, uint8_t *handled)
{
    uint32_t isrReg, regVal;
    uint32_t events;
    ...
    for (qNum=CEDI_PdVar(numQs)-1; qNum>0; qNum--) {
        /* read Interrupt Status Register */
        switch(qNum) {
        default:
        ...
        if (isrReg) {
            ...
            *handled = 1;
            ...
            cond2 = EMAC_REGS__INT_Q_STATUS__RETRY_LIMIT_EXCEEDED_OR_LATE_COLLISION__READ(
                             isrReg);
            ...
            /*************************** RxFrame *******************************/
            if (EMAC_REGS__INT_Q_STATUS__RECEIVE_COMPLETE__READ(isrReg))
            {
                /*vDbgMsg(DBG_GEN_MSG, 10,
                        "EMAC (0x%08X) Rx Frame Complete, queue:%u\n",
                        (uint32_t)CEDI_PdVar(cfg).regBase, qNum);*/
            ...
            /************************ HResp not OK Event ***********************/
            if (EMAC_REGS__INT_Q_STATUS__RESP_NOT_OK__READ(isrReg))
            {
                vDbgMsg(DBG_GEN_MSG, 10,
                        "EMAC (0x%08X) HResp not OK, queue:%u\n",
                        (uint32_t)CEDI_PdVar(cfg).regBase, qNum);
...
/*#ifndef SANITY_CHECK_TESTS
    ...
    if (!(EMAC_REGS__DESIGNCFG_DEBUG12__GEM_HAS_802P3_BR__READ(regVal)))
        config->incExpressTraffic = 0;
    ...
    if (config->incExpressTraffic){
        sumRxDesc += config->eRxQLen + CEDI_MIN_RXBD;
    }
    ...
    sysReq->privDataSize = alignedToPtr(sizeof(CEDI_PrivateData))
                                  + sizeof(uintptr_t)*(sumTxDesc+sumRxDesc);
    if (config->incExpressTraffic){
	/* express mac private data */
	sysReq->privDataSize += alignedToPtr(sizeof(CEDI_PrivateData))
        + ((uint32_t)((config->eTxQLen)+CEDI_MIN_TXBD))*sizeof(uintptr_t)
        + ((uint32_t)((config->eRxQLen)+CEDI_MIN_RXBD))*sizeof(uintptr_t);
    }
...
#define CEDI_ADD_TX_PBUF_SEGS(Q)  if (CEDI_PdVar(numQs)>Q)\
    ...
    uint32_t  paramErr = 0;
    int i;
    uint32_t pmacPDSize, pmacTxDescListSize = 0, pmacRxDescListSize = 0;
    uint16_t txDescSize, rxDescSize;
    CEDI_PrivateData *epD = NULL;
    struct emac_regs *eregBase;

    uint32_t numTxSegs, enumTxSegs, numQSegs, enumQSegs;
    ...
    ((CEDI_PrivateData *)pD)->macType = CEDI_MAC_TYPE_MAC;
    ((CEDI_PrivateData *)pD)->otherMac = NULL;

    if (config->incExpressTraffic){
        pmacPDSize = alignedToPtr(sizeof(CEDI_PrivateData));
        calcDescriptorSizes(config, &txDescSize, &rxDescSize);
        pmacPDSize += numTxDescriptors(config) * sizeof(uintptr_t);
        pmacPDSize += numRxDescriptors(config) * sizeof(uintptr_t);

        pmacTxDescListSize = numTxDescriptors(config) * txDescSize;
        pmacRxDescListSize = numRxDescriptors(config) * rxDescSize;

        ((CEDI_PrivateData *)pD)->macType = CEDI_MAC_TYPE_PMAC;
        ((CEDI_PrivateData *)pD)->otherMac = (CEDI_PrivateData *)(pD + pmacPDSize);
        ((CEDI_PrivateData *)pD)->otherMac->macType = CEDI_MAC_TYPE_EMAC;
        ((CEDI_PrivateData *)pD)->otherMac->otherMac = (CEDI_PrivateData *)pD;
    }
    ...
    if (!paramErr)
    {
        ...
        if (config->incExpressTraffic){
            epD = ((CEDI_PrivateData *)pD)->otherMac;
            epD->cfg = ((CEDI_PrivateData *)pD)->cfg;
            epD->cfg.regBase += CEDI_EXPRESS_MAC_REGS_OFFSET;
            epD->cb = ((CEDI_PrivateData *)pD)->cb;
            epD->cfg.rxQs = 1;
            epD->cfg.txQs = 1;
    ...
    if (!paramErr) {
        ...
        CEDI_PdVar(rxQs) = config->rxQs;
        CEDI_PdVar(txQs) = config->txQs;
        if (config->incExpressTraffic){
    ...
    if (!paramErr) {
        if (0!=(paramErr = config->ifTypeSel>CEDI_IFSP_2500BASE_X))
        {
            vDbgMsg(DBG_GEN_MSG, 5,
        ...
        if (!paramErr && !(is2p5GSupported(pD))) {
            if (0!=(hwConfigErr = config->ifTypeSel>CEDI_IFSP_1000BASE_X))
            {
                vDbgMsg(DBG_GEN_MSG, 5,
    ...
    if ((CEDI_PdVar(numQs)>1) && (CEDI_PdVar(hwCfg).tx_pkt_buffer))
    {
        ...
        if (config->incExpressTraffic){
            //TODO: For eMAC
            enumTxSegs = 1 << epD->hwCfg.tx_pbuf_queue_segment_size;
            enumQSegs = 1 << epD->hwCfg.tx_pbuf_num_segments_q0;
            if (enumQSegs>numTxSegs) {
                vDbgMsg(DBG_GEN_MSG, 5,
   ...
   /****************** Initialise driver internal data ************************/
    ...
    for(i=0; i<CEDI_PdVar(numQs)-1; i++)
        ((CEDI_PrivateData *)pD)->frerEnabled[i] = 0;
    ...
    if (config->incExpressTraffic){
        ...
        initUpper32BuffQAddr(epD);
        if (0!=initTxDescLists(epD))
            return EINVAL;
        if (0!=initRxDescLists(epD))
            return EINVAL;
    }
    ...
    if (config->incExpressTraffic){
        initNetControlReg(epD);
        initNetConfigReg(epD);
        initAxiMaxPipelineReg(epD);
        setRxQBufferSizes(epD, &(epD->cfg));
    }
    ...
    if (config->incExpressTraffic){
        for (i=1; i<=epD->hwCfg.num_spec_add_filters; i++)
            emacDisableSpecAddr(epD, i);
        for (i=0; i<epD->hwCfg.num_type1_screeners-1; i++)
            emacSetType1ScreenReg(epD, i, &clrT1Scrn);
        for (i=0; i<epD->hwCfg.num_type2_screeners-1; i++)
            emacSetType2ScreenReg(epD, i, &clrT2Scrn);
    }
    ...
    if (config->incExpressTraffic){
        emacSetHashAddr(epD, 0, 0);
        emacClearStats(epD);
        emacWriteUserOutputs(epD, 0);
        eregBase = (struct emac_regs *)(epD->cfg.regBase);
        if (EMAC_REGS__NETWORK_CONFIG__PCS_SELECT__READ(
                CPS_UncachedRead32(&(eregBase->network_config)))) {
            emacSetAutoNegEnable(epD, 0);
            epD->anLinkStat = 1;
            epD->anRemFault = 0;
        }
    }
...
void emacDestroy(void *pD)
{
    CEDI_PrivateData *epD = NULL;
    if (pD==NULL) return;
    if (CEDI_PdVar(cfg).incExpressTraffic){
        if (CEDI_MAC_TYPE_EMAC == CEDI_PdVar(macType))
            return;
        epD = ((CEDI_PrivateData *)pD)->otherMac;
        if (epD==NULL) return;
...
void emacStart(void *pD)
{
    ...
    if (CEDI_PdVar(cfg).incExpressTraffic){
        emacSetEventEnable(epD, CEDI_PdVar(cfg).intrEnable, 1, 0);
        emacEnableRx(epD);
        emacEnableTx(epD);
    }
}
...
void emacStop(void *pD)
{
    ...
    if (CEDI_PdVar(cfg).incExpressTraffic){
        emacStopTx(epD);
        for (t=10000; (t && emacTransmitting(epD)); t--);
        if (emacTransmitting(epD))
            emacAbortTx(epD);
...
//          CPS_UncachedRead32(CEDI_RegAddr(int_mask)));
    ...
    if (CEDI_PdVar(cfg).incExpressTraffic){
        if (CEDI_MAC_TYPE_EMAC == CEDI_PdVar(macType))
            return EINVAL;
        epD = ((CEDI_PrivateData *)pD)->otherMac;
        if (epD==NULL) return EINVAL;
    }

    if (CEDI_PdVar(cfg).incExpressTraffic){
        isrReg = CPS_UncachedRead32(CEDI_RegAddr(mmsl_int_status));
        if (isrReg)
        {
            handled = 1;
            if (CEDI_PdVar(cb).mmslEvent)
                (*(CEDI_PdVar(cb).mmslEvent))(pD, isrReg);
            if (0==CEDI_PdVar(hwCfg).irq_read_clear)
                CPS_UncachedWrite32(CEDI_RegAddr(mmsl_int_status), isrReg);
        }
        result = emacHandleGeneralInterupts(epD, &handled);
            if (result) return result;
    }

    result = emacHandleQnInterupts(pD, &handled);
        if (result) return result;
    result = emacHandleGeneralInterupts(pD, &handled);
        if (result) return result;
...
#define CEDI_DISABLE_INT(Q) if ((queueNum==Q) || (queueNum==CEDI_ALL_QUEUES))\
    ...
    if (events)
    {
        if (enable) {
            ...
            if (0!=(paramErr=callbacksNullCheck(pD, events))) {
                vDbgMsg(DBG_GEN_MSG, 5,
                    "*** Error: Callback =NULL for event(s) 0x%08X\n", paramErr);
                return EINVAL;
            }
...
//            __func__, speedSel);
    ...
    if (!(is2p5GSupported(pD)))
    {
        max_gem_speed = CEDI_SPEED_1000M;
    }
    if ((speedSel<CEDI_SPEED_10M) || (speedSel>max_gem_speed))
        return EINVAL;

    reg = CPS_UncachedRead32(CEDI_RegAddr(network_config));
    reg_control = CPS_UncachedRead32(CEDI_RegAddr(network_control));
    ...
    CPS_UncachedWrite32(CEDI_RegAddr(network_control), reg_control);
    if (CEDI_PdVar(cfg).incExpressTraffic){
        reg = CPS_UncachedRead32(CEDI_eRegAddr(network_config));
        reg_control = CPS_UncachedRead32(CEDI_eRegAddr(network_control));
        EMAC_REGS__NETWORK_CONTROL__TWO_PT_FIVE_GIG__CLR(reg_control);
        switch (speedSel) {
        case CEDI_SPEED_10M:
            EMAC_REGS__NETWORK_CONFIG__SPEED__CLR(reg);
        ...
        CPS_UncachedWrite32(CEDI_eRegAddr(network_config), reg);
        CPS_UncachedWrite32(CEDI_eRegAddr(network_control), reg_control);
    }
    return 0;
}
...
uint32_t emacGetIfSpeed(void *pD, CEDI_IfSpeed *speedSel)
{
    ...
    reg = CPS_UncachedRead32(CEDI_RegAddr(network_config));
    reg_control = CPS_UncachedRead32(CEDI_RegAddr(network_control));
    if (EMAC_REGS__NETWORK_CONTROL__TWO_PT_FIVE_GIG__READ(reg_control))
...
uint32_t emacSetUniDirEnable(void *pD, uint8_t enable)
{
    uint32_t reg;
    if (pD==NULL) return EINVAL;
...
uint32_t emacSetFullDuplex(void *pD, uint8_t enable)
{
    uint32_t reg;
    CEDI_PrivateData *epD = NULL;
    if (pD==NULL) return EINVAL;
...
uint32_t emacSetMdioEnable(void *pD, uint8_t enable)
{
    uint32_t ncr;
    if (pD==NULL) return EINVAL;
    if (enable>1) return EINVAL;
 ...
 */
uint32_t emacPhyStartMdioWrite(void *pD, uint8_t flags, uint8_t phyAddr,
        uint8_t devReg, uint16_t addrData)
{
    ...
    if (flags &
        ...
        return EINVAL;
    if (phyAddr > 0x1F) return EINVAL;
    if (devReg > 0x1F) return EINVAL;
    if (CEDI_PdVar(cfg).incExpressTraffic){
        if (CEDI_MAC_TYPE_EMAC == CEDI_PdVar(macType))
        {
            vDbgMsg(DBG_GEN_MSG, 5, "%s\n",
 ...
 *           - enum CEDI_MdioDevType is available to specify the device type
 */
uint32_t emacPhyStartMdioRead(void *pD, uint8_t flags, uint8_t phyAddr,
        uint8_t devReg)
{
    uint32_t reg;
    if (pD==NULL) return EINVAL;
...
uint32_t
emacSetReportingBadFCS(void *pD, uint8_t enable)
{
    uint32_t regTmp;
    if ((pD == NULL) || (enable > 1))
        return EINVAL;
    ...
    regTmp = CPS_UncachedRead32(CEDI_RegAddr(dma_config));
...
uint32_t
emacGetReportingBadFCS(void *pD, uint8_t *enable)
{
    uint32_t regTmp;
    if ((pD == NULL) || (enable == NULL))
        return EINVAL;
    ...
    if (!IsGem1p09(pD))
        return ENOTSUP;
    regTmp = CPS_UncachedRead32(CEDI_RegAddr(dma_config));
    *enable = EMAC_REGS__DMA_CONFIG__CRC_ERROR_REPORT__READ(regTmp);
    return EOK;
#else
    return ENOTSUP;
#endif
...
uint32_t
emacSetPtpSingleStep(void* pD, uint8_t enable)
{
    uint32_t regTmp;
    if ((pD == NULL) || (enable > 1))
        return EINVAL;
    ...
    if (!IsGem1p09(pD))
        return ENOTSUP;
    regTmp = CPS_UncachedRead32(CEDI_RegAddr(network_control));
...
uint32_t
emacGetPtpSingleStep(void* pD, uint8_t *enable)
{
    uint32_t regTmp;
    if ((pD == NULL) || (enable == NULL))
        return EINVAL;
    ...
    if (!IsGem1p09(pD))
        return ENOTSUP;
    regTmp = CPS_UncachedRead32(CEDI_RegAddr(network_control));
    *enable = EMAC_REGS__NETWORK_CONTROL__OSS_CORRECTION_FIELD__READ(regTmp);
    return EOK;
#else
    return ENOTSUP;
#endif
...
uint32_t
emacSetMiiOnRgmii(void *pD, uint8_t enable)
{
    uint32_t regTmp;
    if ((pD == NULL) || (enable > 1))
        return EINVAL;
...
uint32_t
emacGetMiiOnRgmii(void *pD, uint8_t *enable)
{
    uint32_t regTmp;
    if ((pD == NULL) || (enable == NULL))
        return EINVAL;
    ...
    if (!IsGem1p09(pD))
        return ENOTSUP;
    regTmp = CPS_UncachedRead32(CEDI_RegAddr(network_control));
    *enable = EMAC_REGS__NETWORK_CONTROL__SEL_MII_ON_RGMII__READ(regTmp);
    return EOK;
#else
    return ENOTSUP;
#endif
...
uint32_t emacSetFrameEliminationEnable(void* pD, uint8_t queueNum, uint8_t enable)
{
#define CEDI_WR_FRER_EN_N(Q)\
	reg = CPS_UncachedRead32(CEDI_RegAddr(frer_control_##Q##_a));\
	EMAC_REGS__FRER_CONTROL_A__EN_ELIMINATION__MODIFY(reg, enable);\
        CPS_UncachedWrite32(CEDI_RegAddr(frer_control_##Q##_a), reg);\
	break;
    ...
    if (queueNum >= ((CEDI_PrivateData *)pD)->rxQs){
        vDbgMsg(DBG_GEN_MSG, DBG_CRIT, "%s\n",
            "Error: Queue number is bigger than number of active RX queues\n");
        return EINVAL;
    }

    if (!(CEDI_PdVar(hwCfg).num_cb_streams))
        return ENOTSUP;

    if (queueNum >= (((CEDI_PrivateData *)pD)->hwCfg).num_cb_streams){
        vDbgMsg(DBG_GEN_MSG, DBG_CRIT, "%s\n",
            "Error: Queue number is bigger than number of CB streams\n");
        return EINVAL;
    }

    CEDI_PdVar(frerEnabled[queueNum]) = enable;
    ...
    switch(queueNum){
        ...
        CEDI_WR_FRER_EN_N(9);
        case 9:
        CEDI_WR_FRER_EN_N(10);
        case 10:
        CEDI_WR_FRER_EN_N(11);
        case 11:
        CEDI_WR_FRER_EN_N(12);
        case 12:
        CEDI_WR_FRER_EN_N(13);
        case 13:
        CEDI_WR_FRER_EN_N(14);
        case 14:
        CEDI_WR_FRER_EN_N(15);
        case 15:
        CEDI_WR_FRER_EN_N(16);
    }

    return EOK;
}

uint32_t emacGetFrameEliminationEnable(void* pD, uint8_t queueNum, uint8_t *enable)
{
#define CEDI_RD_FRER_EN_N(Q)\
	reg = CPS_UncachedRead32(CEDI_RegAddr(frer_control_##Q##_a));\
	*enable = EMAC_REGS__FRER_CONTROL_A__EN_ELIMINATION__READ(reg);\
	break;
    ...
    switch(queueNum){
        ...
        CEDI_RD_FRER_EN_N(9);
        case 9:
        CEDI_RD_FRER_EN_N(10);
        case 10:
        CEDI_RD_FRER_EN_N(11);
        case 11:
        CEDI_RD_FRER_EN_N(12);
        case 12:
        CEDI_RD_FRER_EN_N(13);
        case 13:
        CEDI_RD_FRER_EN_N(14);
        case 14:
        CEDI_RD_FRER_EN_N(15);
        case 15:
        CEDI_RD_FRER_EN_N(16);
    }

    return EOK;
}

uint32_t emacSetFrameEliminationSeqRecRstTmrEnable(void* pD, uint8_t queueNum, uint8_t enable)
{
#define CEDI_WR_FRER_SRRT_EN_N(Q)\
	reg = CPS_UncachedRead32(CEDI_RegAddr(frer_control_##Q##_a));\
    ...
    switch(queueNum){
        case 0:
        CEDI_WR_FRER_SRRT_EN_N(1);
        case 1:
        CEDI_WR_FRER_SRRT_EN_N(2);
        case 2:
        CEDI_WR_FRER_SRRT_EN_N(3);
        case 3:
        CEDI_WR_FRER_SRRT_EN_N(4);
        case 4:
        CEDI_WR_FRER_SRRT_EN_N(5);
        case 5:
        CEDI_WR_FRER_SRRT_EN_N(6);
        case 6:
        CEDI_WR_FRER_SRRT_EN_N(7);
        case 7:
        CEDI_WR_FRER_SRRT_EN_N(8);
        case 8:
        CEDI_WR_FRER_SRRT_EN_N(9);
        case 9:
        CEDI_WR_FRER_SRRT_EN_N(10);
        case 10:
        CEDI_WR_FRER_SRRT_EN_N(11);
        case 11:
        CEDI_WR_FRER_SRRT_EN_N(12);
        case 12:
        CEDI_WR_FRER_SRRT_EN_N(13);
        case 13:
        CEDI_WR_FRER_SRRT_EN_N(14);
        case 14:
        CEDI_WR_FRER_SRRT_EN_N(15);
        case 15:
        CEDI_WR_FRER_SRRT_EN_N(16);
    }

    return EOK;
}

uint32_t emacGetFrameEliminationSeqRecRstTmrEnable(void* pD, uint8_t queueNum, uint8_t *enable)
{
#define CEDI_RD_FRER_SRRT_EN_N(Q)\
	reg = CPS_UncachedRead32(CEDI_RegAddr(frer_control_##Q##_a));\
	*enable = EMAC_REGS__FRER_CONTROL_A__EN_SEQRECRST_TIMER__READ(reg);\
	break;
    ...
    switch(queueNum){
        case 0:
        CEDI_RD_FRER_SRRT_EN_N(1);
        case 1:
        CEDI_RD_FRER_SRRT_EN_N(2);
        case 2:
        CEDI_RD_FRER_SRRT_EN_N(3);
        case 3:
        CEDI_RD_FRER_SRRT_EN_N(4);
        case 4:
        CEDI_RD_FRER_SRRT_EN_N(5);
        case 5:
        CEDI_RD_FRER_SRRT_EN_N(6);
        case 6:
        CEDI_RD_FRER_SRRT_EN_N(7);
        case 7:
        CEDI_RD_FRER_SRRT_EN_N(8);
        case 8:
        CEDI_RD_FRER_SRRT_EN_N(9);
        case 9:
        CEDI_RD_FRER_SRRT_EN_N(10);
        case 10:
        CEDI_RD_FRER_SRRT_EN_N(11);
        case 11:
        CEDI_RD_FRER_SRRT_EN_N(12);
        case 12:
        CEDI_RD_FRER_SRRT_EN_N(13);
        case 13:
        CEDI_RD_FRER_SRRT_EN_N(14);
        case 14:
        CEDI_RD_FRER_SRRT_EN_N(15);
        case 15:
        CEDI_RD_FRER_SRRT_EN_N(16);
    }
...
uint32_t emacSetFrameEliminationConfig(void* pD, uint8_t queueNum, CEDI_FrameEliminationConfig* fec)
{
#define CEDI_WR_FRER_CONF(Q)\
	reg = CPS_UncachedRead32(CEDI_RegAddr(frer_control_##Q##_a));\
	EMAC_REGS__FRER_CONTROL_A__EN_VECTOR_REC_ALG__MODIFY(reg, !!fec->enVectAlg);\
        ...
        else if (fec->seqNumIdentification == CEDI_SEQ_NUM_IDEN_OFFSET)\
	...
	EMAC_REGS__FRER_CONTROL_A__MEMBER_STREAM_2__MODIFY(reg, fec->smemberStream2);\
        CPS_UncachedWrite32(CEDI_RegAddr(frer_control_##Q##_a), reg);\
	reg = CPS_UncachedRead32(CEDI_RegAddr(frer_control_##Q##_b));\
	...
	EMAC_REGS__FRER_CONTROL_B__SEQ_REC_WINDOW__MODIFY(reg, fec->seqRecWindow);\
        CPS_UncachedWrite32(CEDI_RegAddr(frer_control_##Q##_b), reg);\
	break;
    ...
    if(CEDI_PdVar(frerEnabled[queueNum]))
        return EINVAL;

    if ((fec->enVectAlg > 1) || (fec->seqNumIdentification > 1))
        return EINVAL;

    if ((fec->offsetValue > 511) || (fec->smemberStream1 > 15) || (fec->smemberStream2 > 15))
        return EINVAL;

    if ((fec->seqNumLength > 16) || (fec->seqRecWindow > 63))
        return EINVAL;
    ...
    switch(queueNum){
        ...
        CEDI_WR_FRER_CONF(9);
        case 9:
        CEDI_WR_FRER_CONF(10);
        case 10:
        CEDI_WR_FRER_CONF(11);
        case 11:
        CEDI_WR_FRER_CONF(12);
        case 12:
        CEDI_WR_FRER_CONF(13);
        case 13:
        CEDI_WR_FRER_CONF(14);
        case 14:
        CEDI_WR_FRER_CONF(15);
        case 15:
        CEDI_WR_FRER_CONF(16);
    }

    return EOK;
}

uint32_t emacGetFrameEliminationConfig(void* pD, uint8_t queueNum, CEDI_FrameEliminationConfig* fec)
{
#define CEDI_RD_FRER_CONF(Q)\
	reg = CPS_UncachedRead32(CEDI_RegAddr(frer_control_##Q##_a));\
	fec->enVectAlg = EMAC_REGS__FRER_CONTROL_A__EN_VECTOR_REC_ALG__READ(reg);\
	if (EMAC_REGS__FRER_CONTROL_A__USE_R_TAG__READ(reg))\
	    fec->seqNumIdentification = CEDI_SEQ_NUM_IDEN_TAG;\
	else\
	    fec->seqNumIdentification = CEDI_SEQ_NUM_IDEN_OFFSET;\
	fec->offsetValue = EMAC_REGS__FRER_CONTROL_A__OFFSET_VALUE__READ(reg);\
	fec->smemberStream1 = EMAC_REGS__FRER_CONTROL_A__MEMBER_STREAM_1__READ(reg);\
	fec->smemberStream2 = EMAC_REGS__FRER_CONTROL_A__MEMBER_STREAM_2__READ(reg);\
	reg = CPS_UncachedRead32(CEDI_RegAddr(frer_control_##Q##_b));\
	fec->seqNumLength = EMAC_REGS__FRER_CONTROL_B__SEQ_NUM_LENGTH__READ(reg);\
	fec->seqRecWindow = EMAC_REGS__FRER_CONTROL_B__SEQ_REC_WINDOW__READ(reg);\
	break;
    ...
    switch(queueNum){
        ...
        CEDI_RD_FRER_CONF(9);
        case 9:
        CEDI_RD_FRER_CONF(10);
        case 10:
        CEDI_RD_FRER_CONF(11);
        case 11:
        CEDI_RD_FRER_CONF(12);
        case 12:
        CEDI_RD_FRER_CONF(13);
        case 13:
        CEDI_RD_FRER_CONF(14);
        case 14:
        CEDI_RD_FRER_CONF(15);
        case 15:
        CEDI_RD_FRER_CONF(16);
    }
    return EOK;

}

uint32_t emacSetFrameEliminationTagConfig(void* pD, CEDI_FrameEliminationTagConfig* fetc)
{
    uint32_t reg;
    ...
    reg = CPS_UncachedRead32(CEDI_RegAddr(frer_red_tag));
    ...
    CPS_UncachedWrite32(CEDI_RegAddr(frer_red_tag), reg);

    return EOK;
}

uint32_t emacGetFrameEliminationTagConfig(void* pD, CEDI_FrameEliminationTagConfig* fetc)
{
    uint32_t reg;
    if ((pD == NULL) || (fetc == NULL))
        return EINVAL;
    ...
    reg = CPS_UncachedRead32(CEDI_RegAddr(frer_red_tag));
    fetc->redundancyTag = EMAC_REGS__FRER_RED_TAG__REDUNDANCY_TAG__READ(reg);
    fetc->enStripRedTag = EMAC_REGS__FRER_RED_TAG__STRIP_R_TAG__READ(reg);

    return EOK;
}

uint32_t emacSetFrameEliminationTimoutConfig(void* pD, uint16_t timeout)
{
    uint32_t reg;
    if (pD == NULL)
        return EINVAL;
    ...
    reg = CPS_UncachedRead32(CEDI_RegAddr(frer_timeout));
...
uint32_t emacGetFrameEliminationStats(void* pD, uint8_t queueNum, CEDI_FrameEliminationStats* stats)
{
#define CEDI_RD_FRER_STATS(Q)\
	reg = CPS_UncachedRead32(CEDI_RegAddr(frer_statistics_##Q##_a));\
	stats->latentErrors = EMAC_REGS__FRER_STATISTICS_A__LATENT_ERRORS__READ(reg);\
	stats->vecRecRogue = EMAC_REGS__FRER_STATISTICS_A__VEC_REC_ROGUE__READ(reg);\
	reg = CPS_UncachedRead32(CEDI_RegAddr(frer_statistics_##Q##_b));\
	stats->outOfOreder = EMAC_REGS__FRER_STATISTICS_B__OUT_OF_ORDER__READ(reg);\
	stats->seqRstCount = EMAC_REGS__FRER_STATISTICS_B__SEQRST_COUNT__READ(reg);\
	break;
    ...
    switch(queueNum){
    case 0:
	CEDI_RD_FRER_STATS(1);
    case 1:
	CEDI_RD_FRER_STATS(2);
    case 2:
	CEDI_RD_FRER_STATS(3);
    case 3:
	CEDI_RD_FRER_STATS(4);
    case 4:
	CEDI_RD_FRER_STATS(5);
    case 5:
	CEDI_RD_FRER_STATS(6);
    case 6:
	CEDI_RD_FRER_STATS(7);
    case 7:
	CEDI_RD_FRER_STATS(8);
    case 8:
	CEDI_RD_FRER_STATS(9);
    case 9:
	CEDI_RD_FRER_STATS(10);
    case 10:
	CEDI_RD_FRER_STATS(11);
    case 11:
	CEDI_RD_FRER_STATS(12);
    case 12:
	CEDI_RD_FRER_STATS(13);
    case 13:
	CEDI_RD_FRER_STATS(14);
    case 14:
	CEDI_RD_FRER_STATS(15);
    case 15:
	CEDI_RD_FRER_STATS(16);
    }
    return EOK;
}

uint32_t emacGetEmac(void* pD, void** emacPrivateData)
{
    ...
    switch(CEDI_PdVar(macType)){
    case CEDI_MAC_TYPE_EMAC:
        emacPrivateData = pD;
        break;
    case CEDI_MAC_TYPE_PMAC:
        *emacPrivateData = CEDI_PdVar(otherMac);
        break;
    default:
...
uint32_t emacGetPmac(void* pD, void** pmacPrivateData)
{
    ...
    switch(CEDI_PdVar(macType)){
    case CEDI_MAC_TYPE_EMAC:
        *pmacPrivateData = CEDI_PdVar(otherMac);
        break;
    case CEDI_MAC_TYPE_PMAC:
        *pmacPrivateData = pD;
        break;
...
uint32_t emacGetMacType(void* pD, CEDI_MacType* macType)
{
    ...
    *macType = CEDI_PdVar(macType);

    return 0;
}

uint32_t emacSetPreemptionConfig(void* pD, CEDI_PreemptionConfig* preCfg)
{
    uint32_t reg;
    uint8_t current_ver_enable;
    uint8_t current_add_frag_size;
    uint8_t double_write_required = 0;
    struct emac_regs *mmslRegs;

    if ((pD == NULL) || (preCfg == NULL))
        return EINVAL;

    if ((preCfg->routeRxToPmac > 1) || (preCfg->enPreeption > 1) || (preCfg->enVerify > 1))
        return EINVAL;
    if (preCfg->addFragSize > 3)
        return EINVAL;
...
uint32_t emacGetPreemptionConfig(void* pD, CEDI_PreemptionConfig* preCfg)
{
    uint32_t reg;
    struct emac_regs *mmslRegs;
    ...
    mmslRegs = getMmslRegs(pD);
    reg = CPS_UncachedRead32(&(mmslRegs->mmsl_control));
    preCfg->routeRxToPmac = EMAC_REGS__MMSL_CONTROL__ROUTE_RX_TO_PMAC__READ(reg);
    preCfg->enPreeption = EMAC_REGS__MMSL_CONTROL__PRE_ENABLE__READ(reg);
    preCfg->enVerify = !(EMAC_REGS__MMSL_CONTROL__VERIFY_DISABLE__READ(reg));
    preCfg->addFragSize = EMAC_REGS__MMSL_CONTROL__ADD_FRAG_SIZE__READ(reg);
    return EOK;

}

uint32_t emacPreemptionRestartVerification(void* pD)
{
    uint32_t reg;
    struct emac_regs *mmslRegs;
...
uint32_t emacSetMmslEventEnable(void* pD, uint32_t events, uint8_t enable)
{
    struct emac_regs *mmslRegs;
...
uint32_t emacGetMmslEventEnable(void* pD, uint32_t* events)
{
    uint32_t reg;
    struct emac_regs *mmslRegs;
...
uint32_t emacReadMmslStats(void* pD, CEDI_MmslStats* mmslStats)
{
    uint32_t reg;
    struct emac_regs *mmslRegs;
    ...
    mmslStats->assErrorCount = EMAC_REGS__MMSL_ERR_STATS__ASS_ERROR_COUNT__READ(reg);
    mmslStats->smdErrorCount = EMAC_REGS__MMSL_ERR_STATS__SMD_ERR_COUNT__READ(reg);
...
uint32_t emacReadMmslStatus(void* pD, CEDI_MmslStatus* mmslStatus)
{
    uint32_t reg;
    struct emac_regs *mmslRegs;
    ...
    mmslStatus->preActive = EMAC_REGS__MMSL_STATUS__PRE_ACTIVE__READ(reg);
    ...
    mmslStatus->events = 0;
    if (EMAC_REGS__MMSL_STATUS__RCV_R_ERROR__READ(reg))
        mmslStatus->events |= CEDI_MMSL_EV_RCV_R_ERR;
    if (EMAC_REGS__MMSL_STATUS__RCV_V_ERROR__READ(reg))
        mmslStatus->events |= CEDI_MMSL_EV_RCV_V_ERR;
    if (EMAC_REGS__MMSL_STATUS__SMDS_ERROR__READ(reg))
        mmslStatus->events |= CEDI_MMSL_EV_SMDS_ERR;
    if (EMAC_REGS__MMSL_STATUS__SMDC_ERROR__READ(reg))
        mmslStatus->events |= CEDI_MMSL_EV_SMDC_ERR;
    if (EMAC_REGS__MMSL_STATUS__FRER_COUNT_ERR__READ(reg))
        mmslStatus->events |= CEDI_MMSL_EV_FR_COUNT_ERR;
    if (EMAC_REGS__MMSL_STATUS__SMD_ERROR__READ(reg))
        mmslStatus->events |= CEDI_MMSL_EV_SMD_ERR;
    return EOK;
}

uint32_t emacSetLockupConfig(void* pD, CEDI_LockupConfig* lockupCfg)
{
    uint32_t reg = 0;
    ...
    EMAC_REGS__LOCKUP_CONFIG__LOCKUP_TIME__MODIFY(reg, lockupCfg->lockupTime);
    ...
    CPS_UncachedWrite32(CEDI_RegAddr(lockup_config), reg);

    reg = 0;

    EMAC_REGS__RX_MAC_LOCKUP_TIME__RX_MAC_LOCKUP_TIME__MODIFY(reg, lockupCfg->rxLockupTime);
    CPS_UncachedWrite32(CEDI_RegAddr(rx_mac_lockup_time), reg);

    return EOK;
}

uint32_t emacGetLockupConfig(void* pD, CEDI_LockupConfig* lockupCfg)
{
    uint32_t reg;
    if ((pD==NULL) || (lockupCfg==NULL))
        return EINVAL;
    ...
    reg = CPS_UncachedRead32(CEDI_RegAddr(lockup_config));
    lockupCfg->lockupTime = EMAC_REGS__LOCKUP_CONFIG__LOCKUP_TIME__READ(reg);
    ...
    reg = CPS_UncachedRead32(CEDI_RegAddr(rx_mac_lockup_time));
    lockupCfg->rxLockupTime = EMAC_REGS__RX_MAC_LOCKUP_TIME__RX_MAC_LOCKUP_TIME__READ(reg);

    return EOK;
}

uint32_t emacSetFatalIntSelect(void* pD, CEDI_FatalIntSelect* events)
{
    uint32_t reg = 0;
    if ((pD==NULL) || (events==NULL))
        return EINVAL;
    ...
    EMAC_REGS__FATAL_OR_NON_FATAL_INT_SEL__MANAGEMENT_FRAME_SENT_INT_TYPE__MODIFY(reg, events->managementFrameSent);
    EMAC_REGS__FATAL_OR_NON_FATAL_INT_SEL__RECEIVE_COMPLETE_INT_TYPE__MODIFY(reg, events->receiveComplete);
    EMAC_REGS__FATAL_OR_NON_FATAL_INT_SEL__RX_USED_BIT_READ_INT_TYPE__MODIFY(reg, events->rxUsedBitRead);
    EMAC_REGS__FATAL_OR_NON_FATAL_INT_SEL__TX_USED_BIT_READ_INT_TYPE__MODIFY(reg, events->txUsedBitRead);
    EMAC_REGS__FATAL_OR_NON_FATAL_INT_SEL__TRANSMIT_UNDER_RUN_INT_TYPE__MODIFY(reg, events->transmitUnderrun);
    EMAC_REGS__FATAL_OR_NON_FATAL_INT_SEL__RETRY_LIMIT_EXCEEDED_OR_LATE_COLLISION_INT_TYPE__MODIFY(reg, events->retryLimitExLateColl);
    EMAC_REGS__FATAL_OR_NON_FATAL_INT_SEL__AMBA_ERROR_INT_TYPE__MODIFY(reg, events->ambaError);
    EMAC_REGS__FATAL_OR_NON_FATAL_INT_SEL__TRANSMIT_COMPLETE_INT_TYPE__MODIFY(reg, events->transmitComplete);
    EMAC_REGS__FATAL_OR_NON_FATAL_INT_SEL__LINK_CHANGE_INT_TYPE__MODIFY(reg, events->linkChange);
    EMAC_REGS__FATAL_OR_NON_FATAL_INT_SEL__RECEIVE_OVERRUN_INT_TYPE__MODIFY(reg, events->receiveOverrun);
    EMAC_REGS__FATAL_OR_NON_FATAL_INT_SEL__RESP_NOT_OK_INT_TYPE__MODIFY(reg, events->respNotOk);
    EMAC_REGS__FATAL_OR_NON_FATAL_INT_SEL__PAUSE_FRAME_WITH_NON_ZERO_PAUSE_QUANTUM_RECEIVED_INT_TYPE__MODIFY(reg, events->pauseFrameWithNzQuReceived);
    EMAC_REGS__FATAL_OR_NON_FATAL_INT_SEL__PAUSE_TIME_ELAPSED_INT_TYPE__MODIFY(reg, events->pauseTimeElapsed);
    EMAC_REGS__FATAL_OR_NON_FATAL_INT_SEL__PAUSE_FRAME_TRANSMITTED_INT_TYPE__MODIFY(reg, events->pauseFrameTransmitted);
    EMAC_REGS__FATAL_OR_NON_FATAL_INT_SEL__EXTERNAL_INTERRUPT_INT_TYPE__MODIFY(reg, events->externalInterrupt);
    EMAC_REGS__FATAL_OR_NON_FATAL_INT_SEL__PCS_INT_TYPE__MODIFY(reg, events->pcsInt);
    EMAC_REGS__FATAL_OR_NON_FATAL_INT_SEL__PTP_FRAME_RECEIVED_INT_TYPE__MODIFY(reg, events->ptpFrameReceived);
    EMAC_REGS__FATAL_OR_NON_FATAL_INT_SEL__TSU_SECONDS_REGISTER_INCREMENT_INT_TYPE__MODIFY(reg, events->tsuSecondsRegisterIncrement);
    EMAC_REGS__FATAL_OR_NON_FATAL_INT_SEL__RECEIVE_LPI_INT_TYPE__MODIFY(reg, events->receiveLpi);
    EMAC_REGS__FATAL_OR_NON_FATAL_INT_SEL__WOL_INTERRUPT_INT_TYPE__MODIFY(reg, events->wolInterrupt);
    EMAC_REGS__FATAL_OR_NON_FATAL_INT_SEL__TSU_TIMER_COMPARISON_INTERRUPT_INT_TYPE__MODIFY(reg, events->tsuTimerCompInterrupt);
    EMAC_REGS__FATAL_OR_NON_FATAL_INT_SEL__LOCKUP_DETECTED_INT_TYPE__MODIFY(reg, events->lockupDetected);
    EMAC_REGS__FATAL_OR_NON_FATAL_INT_SEL__RAS_ERR_INT_TYPE__MODIFY(reg, events->rasErr);
    CPS_UncachedWrite32(CEDI_RegAddr(fatal_or_non_fatal_int_sel), reg);

    return EOK;
}

uint32_t emacGetFatalIntSelect(void* pD, CEDI_FatalIntSelect* events)
{
    uint32_t reg = 0;
    if ((pD==NULL) || (events==NULL))
        return EINVAL;
    ...
    reg = CPS_UncachedRead32(CEDI_RegAddr(fatal_or_non_fatal_int_sel));
    events->managementFrameSent = EMAC_REGS__FATAL_OR_NON_FATAL_INT_SEL__MANAGEMENT_FRAME_SENT_INT_TYPE__READ(reg);
    events->receiveComplete = EMAC_REGS__FATAL_OR_NON_FATAL_INT_SEL__RECEIVE_COMPLETE_INT_TYPE__READ(reg);
    events->rxUsedBitRead = EMAC_REGS__FATAL_OR_NON_FATAL_INT_SEL__RX_USED_BIT_READ_INT_TYPE__READ(reg);
    events->txUsedBitRead = EMAC_REGS__FATAL_OR_NON_FATAL_INT_SEL__TX_USED_BIT_READ_INT_TYPE__READ(reg);
    events->transmitUnderrun = EMAC_REGS__FATAL_OR_NON_FATAL_INT_SEL__TRANSMIT_UNDER_RUN_INT_TYPE__READ(reg);
    events->retryLimitExLateColl = EMAC_REGS__FATAL_OR_NON_FATAL_INT_SEL__RETRY_LIMIT_EXCEEDED_OR_LATE_COLLISION_INT_TYPE__READ(reg);
    events->ambaError = EMAC_REGS__FATAL_OR_NON_FATAL_INT_SEL__AMBA_ERROR_INT_TYPE__READ(reg);
    events->transmitComplete = EMAC_REGS__FATAL_OR_NON_FATAL_INT_SEL__TRANSMIT_COMPLETE_INT_TYPE__READ(reg);
    events->linkChange = EMAC_REGS__FATAL_OR_NON_FATAL_INT_SEL__LINK_CHANGE_INT_TYPE__READ(reg);
    events->receiveOverrun = EMAC_REGS__FATAL_OR_NON_FATAL_INT_SEL__RECEIVE_OVERRUN_INT_TYPE__READ(reg);
    events->respNotOk = EMAC_REGS__FATAL_OR_NON_FATAL_INT_SEL__RESP_NOT_OK_INT_TYPE__READ(reg);
    events->pauseFrameWithNzQuReceived = EMAC_REGS__FATAL_OR_NON_FATAL_INT_SEL__PAUSE_FRAME_WITH_NON_ZERO_PAUSE_QUANTUM_RECEIVED_INT_TYPE__READ(reg);
    events->pauseTimeElapsed = EMAC_REGS__FATAL_OR_NON_FATAL_INT_SEL__PAUSE_TIME_ELAPSED_INT_TYPE__READ(reg);
    events->pauseFrameTransmitted = EMAC_REGS__FATAL_OR_NON_FATAL_INT_SEL__PAUSE_FRAME_TRANSMITTED_INT_TYPE__READ(reg);
    events->externalInterrupt = EMAC_REGS__FATAL_OR_NON_FATAL_INT_SEL__EXTERNAL_INTERRUPT_INT_TYPE__READ(reg);
    events->pcsInt = EMAC_REGS__FATAL_OR_NON_FATAL_INT_SEL__PCS_INT_TYPE__READ(reg);
    events->ptpFrameReceived = EMAC_REGS__FATAL_OR_NON_FATAL_INT_SEL__PTP_FRAME_RECEIVED_INT_TYPE__READ(reg);
    events->tsuSecondsRegisterIncrement = EMAC_REGS__FATAL_OR_NON_FATAL_INT_SEL__TSU_SECONDS_REGISTER_INCREMENT_INT_TYPE__READ(reg);
    events->receiveLpi = EMAC_REGS__FATAL_OR_NON_FATAL_INT_SEL__RECEIVE_LPI_INT_TYPE__READ(reg);
    events->wolInterrupt = EMAC_REGS__FATAL_OR_NON_FATAL_INT_SEL__WOL_INTERRUPT_INT_TYPE__READ(reg);
    events->tsuTimerCompInterrupt = EMAC_REGS__FATAL_OR_NON_FATAL_INT_SEL__TSU_TIMER_COMPARISON_INTERRUPT_INT_TYPE__READ(reg);
    events->lockupDetected = EMAC_REGS__FATAL_OR_NON_FATAL_INT_SEL__LOCKUP_DETECTED_INT_TYPE__READ(reg);
    events->rasErr = EMAC_REGS__FATAL_OR_NON_FATAL_INT_SEL__RAS_ERR_INT_TYPE__READ(reg);

    return EOK;
}

uint32_t emacSetRasEventEnable(void* pD, uint32_t events, uint8_t enable)
{
    uint32_t notSuppInterrupts;
    if (pD == NULL)
        return EINVAL;
    ...
    if (enable)
    {
        notSuppInterrupts = ~(CEDI_PdVar(hwCfg).supported_ras_int);
        if (notSuppInterrupts & events)
        {
            vDbgMsg(DBG_GEN_MSG, 5, "%s\n",
                    "*** Warning: Some of requested RAS interrupts are not supported\n");
            return ENOTSUP;
        }
        CPS_UncachedWrite32(CEDI_RegAddr(ras_int_enable), events);
    }
    else
        CPS_UncachedWrite32(CEDI_RegAddr(ras_int_disable), events);
    return EOK;

}

uint32_t emacGetRasEventEnable(void* pD, uint32_t* events)
{
    uint32_t reg;
    uint32_t mask;
    if ((pD == NULL) || (events == NULL))
    ...
    mask = CEDI_PdVar(hwCfg).supported_ras_int;
    reg = CPS_UncachedRead32(CEDI_RegAddr(ras_int_mask));
    *events = (~reg) & mask;
    return EOK;
}

uint32_t emacReadRasStatus(void* pD, uint32_t* rasStatus)
{
    if ((pD == NULL) || (rasStatus == NULL))
        return EINVAL;

    *rasStatus = CPS_UncachedRead32(CEDI_RegAddr(ras_status));

    /* clear statuses by writing back value */
    CPS_UncachedWrite32(CEDI_RegAddr(ras_status), *rasStatus);

    return EOK;

}

uint32_t emacReadRasStats(void* pD, CEDI_RasErrorStats* rasStats)
{
    uint32_t reg;
    if ((pD == NULL) || (rasStats == NULL))
        return EINVAL;

    reg = CPS_UncachedRead32(CEDI_RegAddr(ras_csr_err_count));
    rasStats->csrParityErrCount = EMAC_REGS__RAS_CSR_ERR_COUNT__CSR_PARITY_ERR_COUNT__READ(reg);

    reg = CPS_UncachedRead32(CEDI_RegAddr(ras_dp_err_count));
    rasStats->rxMacDmaDpParityErrCount = EMAC_REGS__RAS_DP_ERR_COUNT__RX_MAC_DMA_DP_PARITY_ERR_COUNT__READ(reg);
    rasStats->rxDmaAxiDpParityErrCount = EMAC_REGS__RAS_DP_ERR_COUNT__RX_DMA_AXI_DP_PARITY_ERR_COUNT__READ(reg);
    rasStats->txDmaMacDpParityErrCount = EMAC_REGS__RAS_DP_ERR_COUNT__TX_DMA_MAC_DP_PARITY_ERR_COUNT__READ(reg);
    rasStats->txAxiDmaDpParityErrCount = EMAC_REGS__RAS_DP_ERR_COUNT__TX_AXI_DMA_DP_PARITY_ERR_COUNT__READ(reg);

    reg = CPS_UncachedRead32(CEDI_RegAddr(ras_ecc_err_count));
    rasStats->eccRxCrrErrCount = EMAC_REGS__RAS_ECC_ERR_COUNT__ECC_RXDPRAM_CRR_ERR_COUNT__READ(reg);
    rasStats->eccRxNcrrErrCount = EMAC_REGS__RAS_ECC_ERR_COUNT__ECC_RXDPRAM_NON_CRR_ERR_COUNT__READ(reg);
    rasStats->eccTxCrrErrCount = EMAC_REGS__RAS_ECC_ERR_COUNT__ECC_TXDPRAM_CRR_ERR_COUNT__READ(reg);
    rasStats->eccTxNcrrErrCount = EMAC_REGS__RAS_ECC_ERR_COUNT__ECC_TXDPRAM_NON_CRR_ERR_COUNT__READ(reg);

    return EOK;

}

uint32_t emacReadTxNcrrErrAddr(void* pD, uint16_t* address)
{
    uint32_t reg;
    if ((pD == NULL) || (address == NULL))
        return EINVAL;

    reg = CPS_UncachedRead32(CEDI_RegAddr(txdpram_non_crr_err_addr));
    *address = EMAC_REGS__TXDPRAM_NON_CRR_ERR_ADDR__TXDPRAM_NON_CRR_ERR_ADDR__READ(reg);

    return EOK;

}

uint32_t emacReadRxNcrrErrAddr(void* pD, uint16_t* address)
{
    uint32_t reg;
    if ((pD == NULL) || (address == NULL))
        return EINVAL;

    reg = CPS_UncachedRead32(CEDI_RegAddr(rxdpram_non_crr_err_addr));
    *address = EMAC_REGS__RXDPRAM_NON_CRR_ERR_ADDR__RXDPRAM_NON_CRR_ERR_ADDR__READ(reg);
...
CEDI_OBJ EmacDrv = {
    ...
    emacSetRxQueueNum,
    emacSetSegAlloc,
    emacSetTxQueueScheduler,
    emacGetTxQueueScheduler,
    emacSetDwrrWeighting,
    emacSetEtsBandAlloc,
    emacGetDwrrWeighting,
    emacGetEtsBandAlloc,
    emacSetEnstTimeConfig,
    emacGetEnstTimeConfig,
    emacSetEnstEnable,
    emacGetEnstEnable,
    emacGetEnstSupported,
    emacSetReportingBadFCS,
    emacGetReportingBadFCS,
    emacSetPtpSingleStep,
    emacGetPtpSingleStep,
    emacSetMiiOnRgmii,
    emacGetMiiOnRgmii,
    emacSetCbsIdleSlope,
    emacGetCbsIdleSlope,
    emacSetFrameEliminationEnable,
    emacGetFrameEliminationEnable,
    emacSetFrameEliminationSeqRecRstTmrEnable,
    emacGetFrameEliminationSeqRecRstTmrEnable,
    emacSetFrameEliminationConfig,
    emacGetFrameEliminationConfig,
    emacSetFrameEliminationTagConfig,
    emacGetFrameEliminationTagConfig,
    emacSetFrameEliminationTimoutConfig,
    emacGetFrameEliminationStats,
    emacGetEmac,
    emacGetPmac,
    emacGetMacType,
    emacSetPreemptionConfig,
    emacGetPreemptionConfig,
    emacPreemptionRestartVerification,
    emacSetMmslEventEnable,
    emacGetMmslEventEnable,
    emacReadMmslStats,
    emacReadMmslStatus,
    emacSetLockupConfig,
    emacGetLockupConfig,
    emacSetFatalIntSelect,
    emacGetFatalIntSelect,
    emacSetRasEventEnable,
    emacGetRasEventEnable,
    emacReadRasStatus,
    emacReadRasStats,
    emacReadTxNcrrErrAddr,
    emacReadRxNcrrErrAddr,
