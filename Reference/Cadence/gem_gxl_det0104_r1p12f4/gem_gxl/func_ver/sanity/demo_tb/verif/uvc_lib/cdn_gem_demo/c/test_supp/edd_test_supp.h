...
#define EMAC_REG_BASE_ADDRESS0  EMAC0_REGS_BASE
#ifdef EMAC1_REGS_BASE
  #define EMAC_REG_BASE_ADDRESS1  EMAC1_REGS_BASE
#endif
...
#define regAddr(gem, reg)  (&(((struct emac_regs *)((((CEDI_PrivateData *)privData[gem])->cfg).regBase))->reg))
#define eRegAddr(gem, reg)  (&(((struct emac_regs *)((((CEDI_PrivateData *)eMacPrivData[gem])->cfg).regBase))->reg))
...
#define validateUint(val,exp,teststr,varstr) \
        { if (val!=exp) { cInst->printf("***** Error %s - %s = %u, expected %u\n",\
                teststr, varstr, val, exp);\
                passed = 0;}}

#define validateXint(val,exp,teststr,varstr) \
        { if (val!=exp) { cInst->printf("***** Error %s - %s = 0x%08X, expected 0x%08X\n",\
                teststr, varstr, val, exp);\
                passed = 0;}}

#define validateNUint(val,exp,teststr,varstr) \
        { if (val==exp) { cInst->printf("***** Error %s - %s = %u, expected not equal to  %u\n",\
                teststr, varstr, val, exp);\
                passed = 0;}}

#define validatePtr(val,exp,teststr,varstr) \
        { if (val!=exp) { cInst->printf("***** Error %s - %s = %p, expected %p\n",\
                teststr, varstr, val, exp);\
                passed = 0;}}
 ...
 */
void txRxTest( void *cddcInst, void *pD, char *paramv );
 ...
 */
void cbTxRxTest( void *cddcInst, void *pD, char *paramv );

/**
 * loopbackTest
 * Test local loopback mode to tx-rx in single GEM instance.
 */
int loopbackTest( void *cddcInst, void *pD, char *paramv );

/**
 * Test transmitting frames using eMAC, when 802.3br is enabled
 */
void emacTxRxTest( void *cddcInst, void *pD, char *paramv );
