...
#define GEM_GXL_MODULE_ID_V2    (0x000A)
#define GEM_XL_MODULE_ID        (0x0008)
#define GEM_AUTO_MODULE_ID_V0   (0x0207)
#define GEM_AUTO_MODULE_ID_V1   (0x020A)
...
/* get register address via the privateDat->cfg.regBase field for express MAC */
#define CEDI_eRegAddr(reg) (&(((struct emac_regs *)(epD->cfg.regBase))->reg))
...
typedef struct CEDI_PrivateData{
...
uint32_t emacSetEnstTimeConfig(void *pD, uint8_t queueNum,
                               CEDI_EnstTimeConfig *enstTimeConfig);

uint32_t emacGetEnstTimeConfig(void *pD, uint8_t queueNum,
                               CEDI_EnstTimeConfig *enstTimeConfig);

uint32_t emacSetEnstEnable(void *pD, uint8_t queue, uint8_t enable);

uint32_t emacGetEnstEnable(void *pD, uint8_t queue, uint8_t *enable);

uint32_t emacGetEnstSupported(void *pD, uint8_t *supported);


uint32_t emacSetReportingBadFCS(void *pD, uint8_t enable);

uint32_t emacGetReportingBadFCS(void *pD, uint8_t *enable);

uint32_t emacSetPtpSingleStep(void* pD, uint8_t enable);

uint32_t emacGetPtpSingleStep(void* pD, uint8_t *enable);

uint32_t emacSetMiiOnRgmii(void *pD, uint8_t enable);

uint32_t emacGetMiiOnRgmii(void *pD, uint8_t *enable);
