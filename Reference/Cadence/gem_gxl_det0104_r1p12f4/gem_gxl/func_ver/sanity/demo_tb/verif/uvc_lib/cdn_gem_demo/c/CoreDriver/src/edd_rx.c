...
{
    if ((pD==NULL) || (numBufs==NULL) || (queueNum>=CEDI_PdVar(rxQs)))
        return EINVAL;
...
uint32_t emacNumRxUsed(void *pD, uint8_t queueNum)
{
    ...
    if ((pD==NULL) || (queueNum>=CEDI_PdVar(rxQs)))
        return 0;
...
//                CEDI_PdVar(cfg).regBase);
	...
	else descData->rxTsData.tsValid = 0;
...
void emacGetRxDescStat(void *pD, uint32_t rxDStatWord, CEDI_RxDescStat *rxDStat)
{
    ...
    network_config = CPS_UncachedRead32(CEDI_RegAddr(network_config));
    dma_config = CPS_UncachedRead32(CEDI_RegAddr(dma_config));
    ...
    if (EMAC_REGS__DMA_CONFIG__HDR_DATA_SPLITTING_EN__READ(dma_config))
    ...
    if (wd1 & CEDI_RXD_EOF){
        ...
        if (EMAC_REGS__DMA_CONFIG__CRC_ERROR_REPORT__READ(dma_config))
            rxDStat->crc = (wd1 & CEDI_RXD_CRC) ? 1 : 0;
        else
#endif
    ...
    if (EMAC_REGS__NETWORK_CONFIG__RECEIVE_CHECKSUM_OFFLOAD_ENABLE__READ(network_config)) {
    ...
    rxDStat->broadcast = (wd1 & CEDI_RXD_BROADCAST_DET)?1:0;
}
...
#define CEDI_WR_RXQ_PTR_REG_N_CASE(Q) case Q:\
        regTmp = CPS_UncachedRead32(CEDI_RegAddr(receive_q##Q##_ptr));\
    ...
    if ((pD==NULL) || (queueNum>=CEDI_PdVar(rxQs)) || (ptrsOnly>1))
        return EINVAL;
    ...
    if (!ptrsOnly) {
        switch (queueNum) {
        case 0:
        regTmp = CPS_UncachedRead32(CEDI_RegAddr(receive_q_ptr));
        /* write hardware base address register */\
        EMAC_REGS__RECEIVE_Q_PTR__DMA_RX_Q_PTR__MODIFY(regTmp, (pAddr >> 2));\
...
uint32_t emacGetRxStatus(void *pD, CEDI_RxStatus *status)
{
    ...
    status->rxMacLockup =
            EMAC_REGS__RECEIVE_STATUS__RX_MAC_LOCKUP_DETECTED__READ(reg);
    status->rxDmaLockup =
            EMAC_REGS__RECEIVE_STATUS__RX_DMA_LOCKUP_DETECTED__READ(reg);
...
uint32_t emacSetRscEnable(void *pD, uint8_t queue, uint8_t enable) {
    ...
    if (CEDI_PdVar(hwCfg).pbuf_rsc==0)
        return ENOTSUP;
    if ((queue<1) || (queue>=(CEDI_PdVar(rxQs))) || (enable>1))
        return EINVAL;
...
uint32_t emacGetRscEnable(void *pD, uint8_t queue, uint8_t *enable) {
    ...
    if ((pD==NULL) || (enable==NULL))
        return EINVAL;
    if ((queue<1)||(queue>=(CEDI_PdVar(rxQs))))
        return EINVAL;
    if (CEDI_PdVar(hwCfg).pbuf_rsc==0)
        return ENOTSUP;
    if (CEDI_MAC_TYPE_EMAC == CEDI_PdVar(macType))
...
static uint32_t QueueCheckAndDisableScreening(void *pD, uint8_t queueIdx)
{
    int i;
    ...
    status = emacGetNumScreenRegs(pD, &numScreeners);
    if (status)
        return status;
    ...
    for (i = 0; i < numScreeners.type1ScrRegs; i++){
        ...
        if ((queueIdx == t1S.qNum) && (t1S.udpEnable || t1S.dstcEnable)){
            t1S.udpEnable = 0;
            t1S.dstcEnable = 0;
            emacSetType1ScreenReg(pD, i, &t1S);
        }
    }
...
uint32_t
emacSetRxQueueNum(void *pD, uint8_t numQueues)
{
    int i;
    uint32_t regTmp;

#define CEDI_WR_RXQ_DIS_CASE(Q, disable) case Q:\
	regTmp = CPS_UncachedRead32(CEDI_RegAddr(receive_q##Q##_ptr));\
    ...
    if (numQueues >  CEDI_PdVar(cfg).rxQs){
    ...
    if (numQueues == ((CEDI_PrivateData *)pD)->rxQs)
        return 0;
    ...
    for (i = numQueues - 1; i <  CEDI_PdVar(cfg).rxQs; i++){
        QueueCheckAndDisableScreening(pD, i);
    }

    /* enable queues if any to enable */
    switch(numQueues){
        CEDI_WR_RXQ_DIS_CASE(15, 0);
        CEDI_WR_RXQ_DIS_CASE(14, 0);
        CEDI_WR_RXQ_DIS_CASE(13, 0);
        CEDI_WR_RXQ_DIS_CASE(12, 0);
        CEDI_WR_RXQ_DIS_CASE(11, 0);
        CEDI_WR_RXQ_DIS_CASE(10, 0);
        CEDI_WR_RXQ_DIS_CASE(9, 0);
        CEDI_WR_RXQ_DIS_CASE(8, 0);
        CEDI_WR_RXQ_DIS_CASE(7, 0);
        CEDI_WR_RXQ_DIS_CASE(6, 0);
        CEDI_WR_RXQ_DIS_CASE(5, 0);
        CEDI_WR_RXQ_DIS_CASE(4, 0);
        CEDI_WR_RXQ_DIS_CASE(3, 0);
        CEDI_WR_RXQ_DIS_CASE(2, 0);
        CEDI_WR_RXQ_DIS_CASE(1, 0);
    }

    /* disable queues if any to disable */
    switch(numQueues){
        CEDI_WR_RXQ_DIS_CASE(1, 1);
        CEDI_WR_RXQ_DIS_CASE(2, 1);
        CEDI_WR_RXQ_DIS_CASE(3, 1);
        CEDI_WR_RXQ_DIS_CASE(4, 1);
        CEDI_WR_RXQ_DIS_CASE(5, 1);
        CEDI_WR_RXQ_DIS_CASE(6, 1);
        CEDI_WR_RXQ_DIS_CASE(7, 1);
        CEDI_WR_RXQ_DIS_CASE(8, 1);
        CEDI_WR_RXQ_DIS_CASE(9, 1);
        CEDI_WR_RXQ_DIS_CASE(10, 1);
        CEDI_WR_RXQ_DIS_CASE(11, 1);
        CEDI_WR_RXQ_DIS_CASE(12, 1);
        CEDI_WR_RXQ_DIS_CASE(13, 1);
        CEDI_WR_RXQ_DIS_CASE(14, 1);
        CEDI_WR_RXQ_DIS_CASE(15, 1);
    }

    /* set new rx priority queues number */
    ((CEDI_PrivateData *)pD)->rxQs = numQueues;
...
uint32_t
emacGetRxQueueNum(void *pD, uint8_t *numQueues)
{
    if ((pD == NULL) || (numQueues == NULL))
        return EINVAL;

    *numQueues = ((CEDI_PrivateData *)pD)->rxQs;
    return 0;
}
