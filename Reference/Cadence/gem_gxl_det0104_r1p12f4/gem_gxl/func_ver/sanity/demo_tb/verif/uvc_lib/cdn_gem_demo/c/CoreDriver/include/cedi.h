/**********************************************************************
 * Copyright (C) 2013-2016 Cadence Design Systems, Inc.
...
#define	CEDI_EXPRESS_MAC_REGS_OFFSET (0x1000)

/**
 * Bit-flags for selecting MMSL-related interrupts to enable/disable/read.
 * @xmlonly <defstofollow>5</defstofollow> REMOVE_ME @endxmlonly
*/
#define	CEDI_MMSL_EV_RCV_R_ERR (0x01)

#define	CEDI_MMSL_EV_RCV_V_ERR (0x02)

#define	CEDI_MMSL_EV_SMDS_ERR (0x04)

#define	CEDI_MMSL_EV_SMDC_ERR (0x08)

#define	CEDI_MMSL_EV_FR_COUNT_ERR (0x10)

#define	CEDI_MMSL_EV_SMD_ERR (0x20)

/**
 * Bit-flags for selecting RAS-related interrupts and statuses to enable/disable/read.
 * @xmlonly <defstofollow>8</defstofollow> REMOVE_ME @endxmlonly
*/
#define	CEDI_RAS_EV_DP_PARITY_ERR (0x01)

#define	CEDI_RAS_EV_CSR_PARITY_ERR (0x02)

#define	CEDI_RAS_EV_TS_PARITY_ERR (0x04)

#define	CEDI_RAS_EV_TSU_ERR (0x08)

#define	CEDI_RAS_EV_DESCR_ADDR_ERR (0x10)

#define	CEDI_RAS_EV_ECC_RX_CRR_ERR (0x20)

#define	CEDI_RAS_EV_ECC_RX_NCRR_ERR (0x40)

#define	CEDI_RAS_EV_ECC_TX_CRR_ERR (0x80)

#define	CEDI_RAS_EV_ECC_TX_NCRR_ERR (0x100)

/** All RAS-related status flags, that contribute to interrupts. */
#define	CEDI_RAS_ALL_INTERRUPTS (0x15F)
...
struct CEDI_EnstTimeConfig;
struct CEDI_DesignCfg;
struct CEDI_Callbacks;
struct CEDI_QueueSegAlloc;
struct CEDI_FrameEliminationConfig;
struct CEDI_FrameEliminationTagConfig;
struct CEDI_FrameEliminationStats;
struct CEDI_PreemptionConfig;
struct CEDI_MmslStats;
struct CEDI_MmslStatus;
struct CEDI_LockupConfig;
struct CEDI_FatalIntSelect;
struct CEDI_RasErrorStats;
...
typedef enum
{
    ...
    CEDI_IFSP_2500M_SGMII = 7,
    CEDI_IFSP_2500BASE_X = 8,
} CEDI_Interface;
...
typedef enum
{
    CEDI_SEGMENTS_PER_QUEUE_1 = 0,
    CEDI_SEGMENTS_PER_QUEUE_2 = 1,
    CEDI_SEGMENTS_PER_QUEUE_4 = 2,
    CEDI_SEGMENTS_PER_QUEUE_8 = 3,
    CEDI_SEGMENTS_PER_QUEUE_16 = 4,
    CEDI_SEGMENTS_PER_QUEUE_32 = 5,
    CEDI_SEGMENTS_PER_QUEUE_64 = 6,
    CEDI_SEGMENTS_PER_QUEUE_128 = 7,
} CEDI_SegmentsPerQueue;

/** Type of scheduling algorithm of transmit queue */
typedef enum
{
    CEDI_TX_SCHED_TYPE_FIXED = 0,
    CEDI_TX_SCHED_TYPE_CBS = 1,
    CEDI_TX_SCHED_TYPE_DWRR = 2,
    CEDI_TX_SCHED_TYPE_ETS = 3,
} CEDI_TxSchedType;
...
{
    /** express MAC */
    CEDI_MAC_TYPE_EMAC = 0,
    /** preemptive MAC */
    CEDI_MAC_TYPE_PMAC = 1,
...
typedef enum
{
    CEDI_SEQ_NUM_IDEN_OFFSET = 0,
    CEDI_SEQ_NUM_IDEN_TAG = 1,
...
typedef enum
{
    ...
    CEDI_FRAG_SIZE_64 = 0,
    /** 128 bytes */
    CEDI_FRAG_SIZE_128 = 1,
    /** 192 bytes */
    CEDI_FRAG_SIZE_192 = 2,
    /** 256 bytes */
    CEDI_FRAG_SIZE_256 = 3,
} CEDI_FragSize;
...
typedef enum
{
    CEDI_INIT_VERIFICATION = 0,
    CEDI_VERIFICATION_IDLE = 1,
    CEDI_SEND_VERIFY = 2,
    CEDI_WAIT_FOR_RESPONSE = 3,
    CEDI_VERIFIED = 4,
    CEDI_VERIFY_FAIL = 5,
} CEDI_VerifyStatus;
...
typedef void (*CEDI_CbMmslEvent)(void* pD, uint32_t events);

/** Callback function for RAS-related interrupts. */
typedef void (*CEDI_CbRasEvent)(void* pD, uint32_t events);
...
typedef struct CEDI_Config
{
    ...
    */
    uintptr_t statsRegs;
    /** if 1 then Interspersing Express Traffic feature will be handled by driver */
...
typedef struct CEDI_EnstTimeConfig
{
    /** time at which queue the queue will start (whole seconds part) */
    uint8_t startTimeS;
    /** time at which queue the queue will start (nanoseconds part) */
    uint32_t startTimeNs;
    /** time in which the gate for a queue has to be open, expressed in bytes */
    uint32_t onTime;
    /** time in which the gate for a queue has to be closed, expressed in bytes */
    uint32_t offTime;
}  CEDI_EnstTimeConfig;
...
typedef struct CEDI_DesignCfg
{
    ...
    uint8_t add_ecc_dpram;
    uint8_t add_dp_parity;
    uint8_t add_csr_parity;
    uint8_t protect_tsu;
    uint8_t protect_descr_addr;
    uint32_t supported_ras_int;
...
*/
typedef struct CEDI_QueueSegAlloc
{
    /**
 ...
 *
*/
typedef struct CEDI_FrameEliminationConfig
{
    /** if 1 enable vector recovery algorithm */
    uint8_t enVectAlg;
    /** selects  what should be used to identify sequence number */
    CEDI_SeqNumIdentification seqNumIdentification;
    /** Offset value in bytes from the start packet delimiter to the most significant byte of the 802.1CB sequence number. (0-511) */
    uint16_t offsetValue;
    /** first screener type 2 register used for member stream identification */
    uint8_t smemberStream1;
    /** second screener type 2 register used for member stream identification */
    uint8_t smemberStream2;
    /** number of significant bits of 802.1CB sequence number. (1-15) */
    uint8_t seqNumLength;
    /** Vector recovery window, defines window size used by the vector recovery algorithm to determine whether to reject a packet. */
    uint8_t seqRecWindow;
}  CEDI_FrameEliminationConfig;

/**
 * Frame elimination tag configuration
 *
*/
typedef struct CEDI_FrameEliminationTagConfig
{
    /** Ethertype value used to identify the redundancy tag */
    uint16_t redundancyTag;
    /** If 1 the redundancy tag is stripped from receive frames. */
    uint8_t enStripRedTag;
}  CEDI_FrameEliminationTagConfig;

/**
 * Frame elimination statistics
 *
*/
typedef struct CEDI_FrameEliminationStats
{
...
}  CEDI_FrameEliminationStats;

/**
 * Configuration of preemption feature
 *
*/
typedef struct CEDI_PreemptionConfig
{
    /** route all received traffic to the pmac rather than the emac when pre_active is 0 in the MMSL status. */
    uint8_t routeRxToPmac;
    /** enabling/disabling the pre-emption operation. */
    uint8_t enPreeption;
    /** enabling/disabling the verification procedure which will determine if the link partner can support 802.3br */
    uint8_t enVerify;
...
}  CEDI_PreemptionConfig;

/**
 * mmsl statistics
 *
*/
typedef struct CEDI_MmslStats
{
 ...
 *
*/
typedef struct CEDI_MmslStatus
{
 ...
 *
*/
typedef struct CEDI_LockupConfig
{
    /** The time after which the lockup detection monitors will trigger, except for the receive MAC lock up detector. */
    uint16_t lockupTime;
    /** The time after which the receive MAC lockup detection monitor will trigger. */
    uint16_t rxLockupTime;
    /** Lockup recovery. If this bit is set then the module will go into a soft reset state if a lockup is detected. */
    ...
    /** Enable the RX MAC lockup detector. Enable the monitor that detects lockups in the MAC transmit path. */
...
/**
 * Selects whether a fatal or non-fatal interrupt is triggered by the ethernet_int signal. If set to one then fatal_interrupt will be triggered for given event.
 *
*/
typedef struct CEDI_FatalIntSelect
{
    uint8_t managementFrameSent:1;
    uint8_t receiveComplete:1;
    uint8_t rxUsedBitRead:1;
    uint8_t txUsedBitRead:1;
    uint8_t transmitUnderrun:1;
    uint8_t retryLimitExLateColl:1;
    uint8_t ambaError:1;
    uint8_t transmitComplete:1;
    uint8_t linkChange:1;
    uint8_t receiveOverrun:1;
    uint8_t respNotOk:1;
    uint8_t pauseFrameWithNzQuReceived:1;
    uint8_t pauseTimeElapsed:1;
    uint8_t pauseFrameTransmitted:1;
    uint8_t externalInterrupt:1;
    uint8_t pcsInt:1;
    uint8_t ptpFrameReceived:1;
    uint8_t tsuSecondsRegisterIncrement:1;
    uint8_t receiveLpi:1;
    uint8_t wolInterrupt:1;
    uint8_t tsuTimerCompInterrupt:1;
    uint8_t lockupDetected:1;
    uint8_t rasErr:1;
}  CEDI_FatalIntSelect;

/**
 * Statistics counters for RAS errors.
 *
*/
typedef struct CEDI_RasErrorStats
{
    /** A count of the number of the number of parity errors that have occurred on all the CSR parity checkers. */
    uint16_t csrParityErrCount;
    /** The number of parity errors occurred on all the parity checkers on the RX data path between the SRAM and MAC output. */
    uint8_t rxMacDmaDpParityErrCount;
    /** The number of parity errors occurred on all the parity checkers on the RX data path between the AXI/AHB and SRAM. */
    uint8_t rxDmaAxiDpParityErrCount;
    /** The number of parity errors occurred on all the parity checkers on the TX data path between the SRAM and MAC output. */
    uint8_t txDmaMacDpParityErrCount;
    /** The number of parity errors occurred on all the parity checkers on the TX data path between the AXI/AHB and SRAM. */
    uint8_t txAxiDmaDpParityErrCount;
    /** The number of correctable errors occurred on ECC RX SRAM checker. */
    uint8_t eccRxCrrErrCount;
    /** The number of non-correctable errors occurred on ECC RX SRAM checker. */
    uint8_t eccRxNcrrErrCount;
    /** The number of correctable errors occurred on ECC TX SRAM checker. */
    uint8_t eccTxCrrErrCount;
    /** The number of non-correctable errors occurred on ECC TX SRAM checker. */
    uint8_t eccTxNcrrErrCount;
}  CEDI_RasErrorStats;
...
typedef struct CEDI_OBJ
{
     ...
     * @return EINVAL if invalid parameter
     */
    uint32_t (*setUniDirEnable)(void* pD, uint8_t enable);
     ...
     * @return EINVAL if invalid parameter
     */
    uint32_t (*setFullDuplex)(void* pD, uint8_t enable);
     ...
     * @return EINVAL if invalid parameter
     */
    uint32_t (*setMdioEnable)(void* pD, uint8_t enable);
     ...
     * @return EINVAL if invalid parameter
     */
    uint32_t (*phyStartMdioWrite)(void* pD, uint8_t flags, uint8_t phyAddr, uint8_t devReg, uint16_t addrData);
     ...
     * @return EINVAL if invalid parameter
     */
    uint32_t (*phyStartMdioRead)(void* pD, uint8_t flags, uint8_t phyAddr, uint8_t devReg);
    ...
    /**
     * Change TX priority queues number
     * @param[in] numQueues number of TX priority queues
     * @param[in] pD driver private state info specific to this instance
     * @return EOK for success
     * @return EINVAL if number of queues is bigger than availble, or pD equal NULL
     */
    uint32_t (*setTxQueueNum)(void* pD, uint8_t numQueues);

    /**
     * Change RX priority queues number
     * @param[in] numQueues number of RX priority queues
     * @param[in] pD driver private state info specific to this instance
     * @return EOK for success
     * @return EINVAL if number of queues is bigger than available, or pD equal NULL
     */
    uint32_t (*setRxQueueNum)(void* pD, uint8_t numQueues);
     ...
     * @param[in] pD driver private state info specific to this instance
     * @return EOK for success
     * @return EINVAL if segment allocation configuration is wrong, or pD equal NULL
     */
    uint32_t (*setTxQueueSegAlloc)(void* pD, CEDI_QueueSegAlloc* queueSegAlloc);
     ...
     * @return EOK for success
     * @return EINVAL if TX queue scheduler configuration is wrong, or pD equal NULL
     */
    uint32_t (*setTxQueueScheduler)(void* pD, uint8_t queueNum, CEDI_TxSchedType schedType);
     ...
     * @return EINVAL for invalid pointer
     */
    uint32_t (*getTxQueueScheduler)(void* pD, uint8_t queueNum, CEDI_TxSchedType* schedType);
     ...
     * @return EINVAL if pD equal NULL
     */
    uint32_t (*setDwrrWeighting)(void* pD, uint8_t queueNum, uint8_t dwrrWeightingVal);
     ...
     * @return EINVAL if pD equal NULL
     */
    uint32_t (*setEtsBandAlloc)(void* pD, uint8_t queueNum, uint8_t etsBandAlloc);
     ...
     * @return EINVAL if pD equal NULL
     */
    uint32_t (*getDwrrWeighting)(void* pD, uint8_t queueNum, uint8_t* dwrrWeightingVal);
     ...
     * @return EINVAL if pD equal NULL
     */
    uint32_t (*getEtsBandAlloc)(void* pD, uint8_t queueNum, uint8_t* etsBandAlloc);
     ...
     * @param[in] queueNum priority queue to configure
     * @return EOK for success
     * @return EINVAL if TX queue time scheduler configuration is wrong, or pD equal NULL
     */
    uint32_t (*setEnstTimeConfig)(void* pD, uint8_t queueNum, CEDI_EnstTimeConfig* enstTimeConfig);
     ...
     * @return EINVAL if pD equal NULL
     */
    uint32_t (*getEnstTimeConfig)(void* pD, uint8_t queueNum, CEDI_EnstTimeConfig* enstTimeConfig);
     ...
     * @param[in] enable if 0 then disable ENST, if 1 then enable ENST
     * @param[in] pD driver private state info specific to this instance
     * @return EOK for success
     * @return EINVAL if pD equal NULL or if ENST is not supported for particular queue
     */
    uint32_t (*setEnstEnable)(void* pD, uint8_t queue, uint8_t enable);
     ...
     * @return EINVAL if pD equal NULL or if ENST is not supported for particular queue
     */
    uint32_t (*getEnstEnable)(void* pD, uint8_t queue, uint8_t* enable);

    /**
     * Functions checks if core supports EnST
     * @param[out] supported value "1" means that EnST is supported. "0" otherwise
     * @param[in] pD driver private state info specific to this instance
     * @return EOK for success
     * @return EINVAL if pD equal NULL or if supported equal NULL
     */
    uint32_t (*getEnstSupported)(void* pD, uint8_t* supported);

    /**
     * function configures report FCS/CRC error in receive buffer
     * descriptor,
     * @param[in] enable if 1 enable reporting FCS/CRC, if 0 disable reporting FCS/CRC
     * @param[in] pD driver private state info specific to this instance
     * @return EOK for success
     * @return EINVAL if pD equal NULL
     */
    uint32_t (*setReportingBadFCS)(void* pD, uint8_t enable);

    /**
     * Function gets state of reporting FCS/CRC error in receive buffer
     * descriptor,
     * @param[out] enable if 1 reporting FCS/CRC i enabled, if 0 reporting FCS/CRC is disabled
     * @param[in] pD driver private state info specific to this instance
     * @return EOK for success
     * @return EINVAL if pD equal NULL
     */
    uint32_t (*getReportingBadFCS)(void* pD, uint8_t* enable);

    /**
     * Function sets enables or disables 1588 One Step Correction Field
     * Update feature in controller,
     * @param[in] enable if 1 enable ptp one step correction, if 0 disable PTP one step correction
     * @param[in] pD driver private state info specific to this instance
     * @return EOK for success
     * @return EINVAL if pD equal NULL
     */
    uint32_t (*setPtpSingleStep)(void* pD, uint8_t enable);

    /**
     * Function gets configuration enables of 1588 One Step Correction
     * Field Update feature
     * @param[out] enable if 1 means that ptp one step correction is enabled, if 0 means that ptp one step correction is disabled
     * @param[in] pD driver private state info specific to this instance
     * @return EOK for success
     * @return EINVAL if pD equal NULL
     */
    uint32_t (*getPtpSingleStep)(void* pD, uint8_t* enable);

    /**
     * Function enables or disables feature whis allows to execute MII
     * operation on RGMII interface
     * @param[in] enable 1 - enable MII operation on RGMII interface, 0 -  enable MII operation on RGMII interface
     * @param[in] pD driver private state info specific to this instance
     * @return EOK for success
     * @return EINVAL if pD equal NULL
     */
    uint32_t (*setMiiOnRgmii)(void* pD, uint8_t enable);
     ...
     * @return EINVAL if pD equal NULL
     */
    uint32_t (*getMiiOnRgmii)(void* pD, uint8_t* enable);

    /**
     * Function sets a new idle slope value for the queue.
     * @param[in] idleSlope new idle slop value
     * @param[in] pD driver private state info specific to this instance
     * @param[in] queueNum priority queue to set new idle slop value
     * @return EOK for success
     * @return EINVAL if there is only one active queue
     */
    uint32_t (*setCbsIdleSlope)(void* pD, uint8_t queueNum, uint32_t idleSlope);
     ...
     * @return EINVAL if there is only one active queue
     */
    uint32_t (*getCbsIdleSlope)(void* pD, uint8_t queueNum, uint32_t* idleSlope);
     ...
     * @return EINVAL for invalid queueNum
     */
    uint32_t (*setFrameEliminationEnable)(void* pD, uint8_t queueNum, uint8_t enable);
     ...
     * @return EINVAL for invalid queueNum
     */
    uint32_t (*getFrameEliminationEnable)(void* pD, uint8_t queueNum, uint8_t* enable);

    /**
     * Function enable/disable sequence recovery reset timer for
     * elimination of received frames functionality
     * @param[in] enable 1 - enable sequence recovery reset timer. 0 otherwise.
     * @param[in] pD driver private state info specific to this instance
     * @param[in] queueNum priority queue to enable disable  elimination frames
     * @return EOK for success
     * @return EINVAL for invalid queueNum
     */
    uint32_t (*setFrameEliminationSeqRecRstTmrEnable)(void* pD, uint8_t queueNum, uint8_t enable);
     ...
     * @return EINVAL for invalid queueNum
     */
    uint32_t (*getFrameEliminationSeqRecRstTmrEnable)(void* pD, uint8_t queueNum, uint8_t* enable);
     ...
     * @return EINVAL for invalid queueNum
     */
    uint32_t (*setFrameEliminationConfig)(void* pD, uint8_t queueNum, CEDI_FrameEliminationConfig* fec);
     ...
     * @return EINVAL for invalid queueNum
     */
    uint32_t (*getFrameEliminationConfig)(void* pD, uint8_t queueNum, CEDI_FrameEliminationConfig* fec);
     ...
     * @param[in] pD driver private state info specific to this instance
     * @return EOK for success
     * @return EINVAL if fetc equal NULL
     */
    uint32_t (*setFrameEliminationTagConfig)(void* pD, CEDI_FrameEliminationTagConfig* fetc);
     ...
     * @return EINVAL if fetc equal NULL
     */
    uint32_t (*getFrameEliminationTagConfig)(void* pD, CEDI_FrameEliminationTagConfig* fetc);
     ...
     * @return EINVAL if pD equal NULL
     */
    uint32_t (*setFrameEliminationTimoutConfig)(void* pD, uint16_t timeout);
     ...
     * @return EINVAL for invalid queueNum
     */
    uint32_t (*getFrameEliminationStats)(void* pD, uint8_t queueNum, CEDI_FrameEliminationStats* stats);
     ...
     * @return EINVAL if pD equal NULL
     */
    uint32_t (*getEmac)(void* pD, void** emacPrivateData);
     ...
     * @return EINVAL if pD equal NULL
     */
    uint32_t (*getPmac)(void* pD, void** pmacPrivateData);
     ...
     * @param[in] pD driver private state info specific to this instance
     * @return EOK for success
     * @return EINVAL if macType equal NULL
     */
    uint32_t (*getMacType)(void* pD, CEDI_MacType* macType);
     ...
     * @param[in] pD driver private state info specific to this instance
     * @return EOK for success
     * @return EINVAL if preCfg equal NULL
     */
    uint32_t (*setPreemptionConfig)(void* pD, CEDI_PreemptionConfig* preCfg);
     ...
     * @return EINVAL if preCfg equal NULL
     */
    uint32_t (*getPreemptionConfig)(void* pD, CEDI_PreemptionConfig* preCfg);
     ...
     * @return EINVAL if pD equal NULL
     */
    uint32_t (*preemptionRestartVerification)(void* pD);
     ...
     * @return EINVAL if pD equal NULL
     */
    uint32_t (*setMmslEventEnable)(void* pD, uint32_t events, uint8_t enable);
     ...
     * @return EINVAL if pD equal NULL
     */
    uint32_t (*getMmslEventEnable)(void* pD, uint32_t* events);
     ...
     * @return EINVAL if pD equal NULL
     */
    uint32_t (*readMmslStats)(void* pD, CEDI_MmslStats* mmslStats);
     ...
     * @return EINVAL if pD equal NULL
     */
    uint32_t (*readMmslStatus)(void* pD, CEDI_MmslStatus* mmslStatus);
     ...
     * @param[in] pD driver private state info specific to this instance
     * @return EOK for success
     * @return EINVAL if lockupCfg equal NULL
     */
    uint32_t (*setLockupConfig)(void* pD, CEDI_LockupConfig* lockupCfg);
     ...
     * @return EINVAL if lockupCfg equal NULL
     */
    uint32_t (*getLockupConfig)(void* pD, CEDI_LockupConfig* lockupCfg);

    /**
     * Function sets lockup detection and recovery configuration
     * @param[in] fatalIntCfg Selection of interrupts to be treated as fatal.
     * @param[in] pD driver private state info specific to this instance
     * @return EOK for success
     * @return EINVAL if fatalIntCfg equal NULL
     */
    uint32_t (*setFatalIntSelect)(void* pD, CEDI_FatalIntSelect* fatalIntCfg);

    /**
     * Function gets lockup detection and recovery configuration
     * @param[out] fatalIntCfg Selection of interrupts to be treated as fatal.
     * @param[in] pD driver private state info specific to this instance
     * @return EOK for success
     * @return EINVAL if fatalIntCfg equal NULL
     */
    uint32_t (*getFatalIntSelect)(void* pD, CEDI_FatalIntSelect* fatalIntCfg);

    /**
     * Enable or disable the specified RAS interrupts
     * @param[in] enable if =1 enable the events, if =0 then disable
     * @param[in] events OR'd combination of bit-flags selecting the events to be enabled or disabled
     * @param[in] pD driver private state info specific to this instance
     * @return EOK for success
     * @return EINVAL if pD equal NULL
     */
    uint32_t (*setRasEventEnable)(void* pD, uint32_t events, uint8_t enable);

    /**
     * Read the specified RAS interrupts OR the returned event value with
     ...
     * @return EINVAL if pD equal NULL
     */
    uint32_t (*getRasEventEnable)(void* pD, uint32_t* events);

    /**
     * reads RAS status
     * @param[out] rasStatus RAS status
     * @param[in] pD driver private state info specific to this instance
     * @return EOK for success
     * @return ENOTSUP if no_stats defined
     * @return EINVAL if pD equal NULL
     */
    uint32_t (*readRasStatus)(void* pD, uint32_t* rasStatus);

    /**
     * reads RAS statistics
     * @param[out] rasStats RAS statistics
     * @param[in] pD driver private state info specific to this instance
     * @return EOK for success
     * @return ENOTSUP if no_stats defined
     * @return EINVAL if pD equal NULL
     */
    uint32_t (*readRasStats)(void* pD, CEDI_RasErrorStats* rasStats);

    /**
     * reads the TX SRAM offset address that caused the non-correctable
     * error.
     * @param[in] pD driver private state info specific to this instance
     * @param[out] address Address that caused error. Not overwritten by subsequent detected errors until read.
     * @return EOK for success
     * @return ENOTSUP if no_stats defined
     * @return EINVAL if pD equal NULL
     */
    uint32_t (*readTxNcrrErrAddr)(void* pD, uint16_t* address);

    /**
     * reads the RX SRAM offset address that caused the non-correctable
     * error.
     * @param[in] pD driver private state info specific to this instance
     * @param[out] address Address that caused error. Not overwritten by subsequent detected errors until read.
     * @return EOK for success
     ...
     * @return EINVAL if pD equal NULL
     */
    uint32_t (*readRxNcrrErrAddr)(void* pD, uint16_t* address);
