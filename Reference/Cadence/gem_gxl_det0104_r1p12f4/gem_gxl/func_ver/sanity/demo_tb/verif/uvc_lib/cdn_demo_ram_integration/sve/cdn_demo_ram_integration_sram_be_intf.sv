...
//----------------------------------------------------------------------------
// Description :
// This file defines the address, data and control interface for the SRAM with byte enables.
//----------------------------------------------------------------------------

interface cdn_demo_ram_integration_sram_be_intf #(parameter ADDR_WD=1, parameter DATA_WD=1) (input logic clk);
  // Byte Number - must match Data Width
  localparam  BN = DATA_WD/8;
...
`endif
  ...
  logic [DATA_WD/8-1:0] ben  = {(DATA_WD/8){1'b0}};
  logic                 en   = 1'b0;
  logic                 we   = 1'b0;
...
*/
  task write (input logic [ADDR_WD-1  :0] waddr,
              ...
              input logic [DATA_WD/8-1:0] wben);
  @(cb);
    cb.addr  <= waddr;
    cb.ben   <= wben;
  //cb.din   <= wdata;
