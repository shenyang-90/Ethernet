...
uint8_t lockupRx[2];
uint8_t lockupTx[2];
uint8_t oldLockupRx[2];
uint8_t oldLockupTx[2];
...
void printTxDescList( cddcOp *cInst, CEDI_PrivateData *pD, uint8_t qNum)
{
  ...
  txQ = &(pD->txQueue[qNum]);
  thisDesc = txQ->bdBase;
  numWd = (pD->txDescriptorSize)>>CEDI_BYTES_PER_WORD_SHIFT;
  for (i=0; i<(pD->cfg.txQLen)[qNum]+1; i++) {
	...
	cInst->printf("u:%u w:%u"\
      ...
      thisDesc = (txDesc *)
    		  (((uintptr_t)thisDesc) + (pD->rxDescriptorSize));
  }
  cInst->printf("\n");
#endif
}
...
void printRxDescList(cddcOp *cInst, CEDI_PrivateData *pD, uint8_t qNum)
{
  ...
  numWd = (pD->rxDescriptorSize)>>CEDI_BYTES_PER_WORD_SHIFT;
  cInst->printf("\n---  Rx Descriptor List Queue %u  ---\n", qNum);
  for (i=0; i<=(pD->cfg.rxQLen)[qNum]; i++) {

	  descCounter = (rxDesc*)((uint32_t)(pD->rxQueue[qNum].rxDescStart)
	                              + i*(pD->rxDescriptorSize));
	  cInst->printf("d%3u ",i);
      ...
      if (wd[0] & CEDI_RXD_USED) {
          cInst->printf("    d[0]=0x%08X d[1]=0x%08X d[2]=0x%08X d[3]=0x%08X\n",
                  CPS_UncachedRead32((uint32_t *)((pD->rxQueue[qNum].rxBufVAddr)[i])),
                  CPS_UncachedRead32((uint32_t *)((pD->rxQueue[qNum].rxBufVAddr)[i])+1),
                  CPS_UncachedRead32((uint32_t *)((pD->rxQueue[qNum].rxBufVAddr)[i])+2),
                  CPS_UncachedRead32((uint32_t *)((pD->rxQueue[qNum].rxBufVAddr)[i])+3));
          cInst->printf("    d[4]=0x%08X d[5]=0x%08X d[6]=0x%08X d[7]=0x%08X\n",
                  CPS_UncachedRead32((uint32_t *)((pD->rxQueue[qNum].rxBufVAddr)[i])+4),
                  CPS_UncachedRead32((uint32_t *)((pD->rxQueue[qNum].rxBufVAddr)[i])+5),
                  CPS_UncachedRead32((uint32_t *)((pD->rxQueue[qNum].rxBufVAddr)[i])+6),
                  CPS_UncachedRead32((uint32_t *)((pD->rxQueue[qNum].rxBufVAddr)[i])+7));
          cInst->printf("    d[12]=0x%08X d[13]=0x%08X d[14]=0x%08X d[15]=0x%08X\n",
                  CPS_UncachedRead32((uint32_t *)((pD->rxQueue[qNum].rxBufVAddr)[i])+12),
                  CPS_UncachedRead32((uint32_t *)((pD->rxQueue[qNum].rxBufVAddr)[i])+13),
                  CPS_UncachedRead32((uint32_t *)((pD->rxQueue[qNum].rxBufVAddr)[i])+14),
                  CPS_UncachedRead32((uint32_t *)((pD->rxQueue[qNum].rxBufVAddr)[i])+15));
      }
#endif
      thisVP++;
  }
...
void printDesignCfg(cddcOp *cInst, CEDI_PrivateData *pD)
{
  ...
  cInst->printf("   pbuf_lso:%u  ", EMAC_REGS__DESIGNCFG_DEBUG6__PBUF_LSO__READ(reg));
...
uint32_t detectCallback(cddcOp *cInst, uint8_t mac) {
    ...
    if (lockupRx[mac]>oldLockupRx[mac]) {
        oldLockupRx[mac]++;
        change = 1;
        cInst->printf("--- MAC%u lockupRx cb counted\n", mac);
    }
    if (lockupTx[mac]>oldLockupTx[mac]) {
        oldLockupTx[mac]++;
        change = 1;
        cInst->printf("--- MAC%u lockupTx cb counted\n", mac);
    }
 ...
 */
static void _ev_ptpPriFrameTx(CEDI_PrivateData *pD,
        uint32_t frType,
 ...
 * @param   time        PTP timer value
 */
static void _ev_ptpPeerFrameTx(CEDI_PrivateData *pD,
        uint32_t frType,
        CEDI_1588TimerVal *time) {
 ...
 * @param   time        PTP timer value
 */
static void _ev_ptpPriFrameRx(CEDI_PrivateData *pD,
        uint32_t frType,
        CEDI_1588TimerVal *time) {
 ...
 * @param   time        PTP timer value
 */
static void _ev_ptpPeerFrameRx(CEDI_PrivateData *pD,
        uint32_t frType,
        CEDI_1588TimerVal *time) {
