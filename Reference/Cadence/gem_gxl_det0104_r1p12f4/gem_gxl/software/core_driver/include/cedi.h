...
#define	CEDI_EV_RX_LOCKUP (0x40000000)

/** TX lockup has been detected */
#define	CEDI_EV_TX_LOCKUP (0x80000000)
...
#define	CEDI_TXB_ENABLE_TLT (0x40)

/** Max length accepted by queueTxBuf */
#define	CEDI_TXD_LMASK (0x4000 - 1)
...
struct CEDI_TransmitLaunchTime;
...
struct CEDI_RxQFlushConfig;
struct CEDI_Type2ScreenerRateLimit;
struct CEDI_AxiQosConfig;
struct CEDI_AsfInfo;
...
typedef enum
{
    CEDI_LINK_FAULT_INDICATION_OK = 0,
    CEDI_LINK_FAULT_INDICATION_LOCAL_FAULT = 1,
    CEDI_LINK_FAULT_INDICATION_REMOTE_FAULT = 2,
    CEDI_LINK_FAULT_INDICATION_LINK_INTERRUPTION = 3,
} CEDI_LinkFaultIndication;

typedef enum
{
    CEDI_REDUNDANCY_TAG_SIZE_4 = 0,
    CEDI_REDUNDANCY_TAG_SIZE_6 = 1,
...
typedef enum
{
    ...
    CEDI_T2COMP_OFF_VID_C_TAG = 4,
    CEDI_T2COMP_OFF_VID_S_TAG = 5,
} CEDI_T2Offset;
...
typedef enum
{
    ...
    CEDI_FLUSH_MODE_OFF = 0,
    /** Mode depending on received bytes. */
    CEDI_FLUSH_MODE_LIMIT_NUM_BYTES = 1,
    /** Mode depending on received frames size. */
    CEDI_FLUSH_MODE_LIMIT_FRAME_SIZE = 2,
} CEDI_RxQFlushMode;
...
*/
typedef void (*CEDI_CbPhyManComplete)(struct CEDI_PrivateData* pD, uint8_t read, uint16_t readData);
 ...
 * range 0 to txQs-1.
*/
typedef void (*CEDI_CbTxEvent)(struct CEDI_PrivateData* pD, uint32_t event, uint8_t queueNum);
...
*/
typedef void (*CEDI_CbTxError)(struct CEDI_PrivateData* pD, uint32_t error, uint8_t queueNum);

/**
 * Reports Frame Rx completed successfully.  Param: queueNum - the priority
 * queue number, range 0 to rxQs-1.
*/
typedef void (*CEDI_CbRxFrame)(struct CEDI_PrivateData* pD, uint8_t queueNum);
...
*/
typedef void (*CEDI_CbRxError)(struct CEDI_PrivateData* pD, uint32_t error, uint8_t queueNum);

/**
 * Reports Hresp not OK error.  Param: queueNum - number of the Tx or Rx
 * queue being accessed (range 0 to rxQs-1).
*/
typedef void (*CEDI_CbHrespError)(struct CEDI_PrivateData* pD, uint8_t queueNum);
...
*/
typedef void (*CEDI_CbLpPageRx)(struct CEDI_PrivateData* pD, struct CEDI_LpPageRx* pageRx);

/**
 * Reports PCS auto-negotiation completed. Param: netStat - pointer to a
 * struct with the network resolution status.
*/
typedef void (*CEDI_CbAnComplete)(struct CEDI_PrivateData* pD, struct CEDI_NetAnStatus* netStat);
...
*/
typedef void (*CEDI_CbLinkChange)(struct CEDI_PrivateData* pD, uint8_t linkState);
...
*/
typedef void (*CEDI_CbLockupEvent)(struct CEDI_PrivateData* pD, uint32_t event);
...
*/
typedef void (*CEDI_CbTsuEvent)(struct CEDI_PrivateData* pD, uint32_t event);
...
*/
typedef void (*CEDI_CbPauseEvent)(struct CEDI_PrivateData* pD, uint32_t event);
...
*/
typedef void (*CEDI_CbPtpPriFrameTx)(struct CEDI_PrivateData* pD, uint32_t type, struct CEDI_1588TimerVal* timeVal);
 ...
 * transmitted.  This is not set if IP config param tsu=0.
*/
typedef void (*CEDI_CbPtpPeerFrameTx)(struct CEDI_PrivateData* pD, uint32_t type, struct CEDI_1588TimerVal* timeVal);
...
*/
typedef void (*CEDI_CbPtpPriFrameRx)(struct CEDI_PrivateData* pD, uint32_t type, struct CEDI_1588TimerVal* timeVal);
 ...
 * received.  This is not set if IP config param tsu=0.
*/
typedef void (*CEDI_CbPtpPeerFrameRx)(struct CEDI_PrivateData* pD, uint32_t type, struct CEDI_1588TimerVal* timeVal);

/** Reports LPI Status changed. */
typedef void (*CEDI_CbLpiStatus)(struct CEDI_PrivateData* pD);

/** Reports Wake On LAN event received. */
typedef void (*CEDI_CbWolEvent)(struct CEDI_PrivateData* pD);

/** Reports External input interrupt occurred. */
typedef void (*CEDI_CbExtInpIntr)(struct CEDI_PrivateData* pD);

/** Callback function for MMSL-related interrupts. */
typedef void (*CEDI_CbMmslEvent)(struct CEDI_PrivateData* pD, uint32_t events);
...
typedef struct CEDI_PrivateData CEDI_PrivateData;
...
typedef struct CEDI_qTxBufParams
{
    ...
    struct CEDI_TransmitLaunchTime launchTime;
}  CEDI_qTxBufParams;
...
{
    /** defines a prescaler value which is the number of tx_clk periods used to scale the dmaLockupTime, txLockupTime and rxLockupTime */
    uint16_t prescaler;
    /** Time after which transmit and receive DMA lockup detection */
    uint16_t dmaLockupTime;
    /** The time after which the transmit  MAC lockup detection monitor will trigger. */
    ...
    /** ORed combination of bit flags DMA TX lockup timer for each queue (bit 0 - timer for queue 0, bit 15 - timer for queue 15). Set bit means that timer is enabled. */
    uint16_t enTxDmaTimers;
}  CEDI_LockupConfig;

/** Receive queue flushing configuration. */
typedef struct CEDI_RxQFlushConfig
{
    /** If set to 1, all frames for this queue will be dropped. */
    uint8_t dropAllFrames;
    /** If set to 1, and free DMA descriptor for queue cannot be obtained, frames will be dropped. */
    uint8_t dropOnRsrcErr;
    /** Used to select queue flushing mode isung limits. */
    CEDI_RxQFlushMode rxQFlushMode;
    /** Used for receive queue flushing modes using limits. */
    uint16_t maxVal;
}  CEDI_RxQFlushConfig;

/** Used to configure max receive rate for type 2 screening register. */
typedef struct CEDI_Type2ScreenerRateLimit
{
    /** Interval time at which number of received bytes is compared against max_rate_val. */
    uint16_t intervalTime;
    /** Maximium number of bytes screener is permitted to match in interval time. */
    uint16_t maxRateVal;
}  CEDI_Type2ScreenerRateLimit;

/** AXI quality of service configuration */
typedef struct CEDI_AxiQosConfig
{
...
typedef struct CEDI_OBJ
{
     ...
     * @param[in] txIntFrameThreshold Count of frames to transmit before assering interrupt.
     * @param[in] rxIntFrameThreshold Count of frames to receive before assering interrupt.
     * @param[in] pD driver private state info specific to this instance
     * @return EOK for success
     * @return ENOTSUP if feature is not available
     * @return EINVAL for invalid pointer
     */
     ...
     * @param[in] pD driver private state info specific to this instance
     * @return EOK if successful
     * @return ENOTSUP if CEDI_TXB_ENABLE_TLT is selected and Time Base Scheduling is not supported
     * @return ENOENT if no available descriptors
     ...
     * @param[in] pD driver private state info specific to this instance
     */
    void (*getTxDescSize)(const CEDI_PrivateData* pD, uint32_t* txDescSize);
    /**
     ...
     * @param[in] pD driver private state info specific to this instance
     */
    void (*stopTx)(CEDI_PrivateData* pD);
    /**
     ...
     */
    uint32_t (*getTxStatus)(CEDI_PrivateData* pD, CEDI_TxStatus* status);
    /**
     ...
     * @param[in] pD driver private state info specific to this instance
     */
    void (*clearTxStatus)(CEDI_PrivateData* pD, uint32_t resetStatus);
    /**
     ...
     * @param[in] pD driver private state info specific to this instance
     */
    void (*getRxDescSize)(const CEDI_PrivateData* pD, uint32_t* rxDescSize);
    /**
     ...
     * @param[in] pD driver private state info specific to this instance
     */
    void (*clearRxStatus)(CEDI_PrivateData* pD, uint32_t resetStatus);
    /**
     ...
     */
    uint32_t (*disableSpecAddr)(CEDI_PrivateData* pD, uint8_t addrNum);
    /**
     ...
     * @return EINVAL if pD equal NULL
     */
    uint32_t (*setHashAddr)(CEDI_PrivateData* pD, uint32_t hAddrTop, uint32_t hAddrBot);
    /**
     ...
     * @return EINVAL if any parameter equal NULL
     */
    uint32_t (*getHashAddr)(CEDI_PrivateData* pD, uint32_t* hAddrTop, uint32_t* hAddrBot);
    /**
     ...
     * @return EINVAL if parameter invalid
     */
    uint32_t (*setType1ScreenReg)(CEDI_PrivateData* pD, uint8_t regNum, const CEDI_T1Screen* regVals);
    /**
     ...
     * @return EINVAL if parameter invalid
     */
    uint32_t (*getType1ScreenReg)(CEDI_PrivateData* pD, uint8_t regNum, CEDI_T1Screen* regVals);
    /**
     ...
     * @return EINVAL if parameter invalid
     */
    uint32_t (*setType2ScreenReg)(CEDI_PrivateData* pD, uint8_t regNum, const CEDI_T2Screen* regVals);
    /**
     ...
     * @return EINVAL if parameter invalid
     */
    uint32_t (*getType2ScreenReg)(CEDI_PrivateData* pD, uint8_t regNum, CEDI_T2Screen* regVals);
    /**
     ...
     * @return ENOTSUP if no_stats defined
     */
    uint32_t (*clearStats)(CEDI_PrivateData* pD);
    /**
     ...
     * @return EOK for success
     * @return ENOTSUP if feature is not available
     * @return EINVAL for invalid linkFault
     */
     ...
     * @return EOK for success
     * @return ENOTSUP if feature is not available
     * @return EINVAL for invalid tagSize
     */
     ...
     * @return EOK for success
     * @return ENOTSUP if feature is not available
     * @return EINVAL if tagSize equal NULL
     */
     ...
     * register.
     * @param[in] regNum Type 1 register number, range 0 to CEDI_DesignCfg.num_type1_screeners-1
     * @param[in] enable if =1 then enable dropping, if =0 then disable dropping.
     * @param[in] pD driver private state info specific to this instance
     * @return EOK for success
     * @return ENOTSUP if feature is not available
     * @return EINVAL for invalid arguments values or NULL pointers
     */
     ...
     * @param[in] regBase base address of MAC registers
     * @return EOK on success
     * @return EINVAL if config is NULL, or hardware not present
     */
    uint32_t (*getAsfInfo)(uintptr_t regBase, CEDI_AsfInfo* asfInfo);
} CEDI_OBJ;
