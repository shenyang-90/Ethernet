...
| # | 检查项 | 通过标准 | 结果 |
|---|--------|----------|:----:|
| 1 | TLM 建模计划 | 模型范围、抽象级别 (AT/LT)、交付时间与责任人明确，已纳入 PAD 补完计划 | ⬜ 待检查 |
| 2 | TLM 平台可编译 | TLM 平台在目标工具链 (SystemC/编译器) 下零错误编译通过，构建脚本可一键复现 | ⬜ 待检查 |
| 3 | 关键场景覆盖 | 覆盖 PAD 关键用例：多端口并发转发、PTP 时间戳、TSN 调度、DMA 吞吐 | ⬜ 待检查 |
| 4 | 指标可量化 | 带宽/时延/资源占用等指标可量化输出，并可与 Arch Spec 架构目标对比 | ⬜ 待检查 |
| 5 | 假设一致性 | TLM 模型假设与 Arch Spec / Interface Spec 一致，任何偏差均有书面记录 | ⬜ 待检查 |
| 6 | 资产移交 | 模型源码、构建脚本、使用文档归档至项目仓库，可供 EDR 阶段直接复用 | ⬜ 待检查 |
